...
static void b_eml_xaxpy(int32_T n, real_T a, const emxArray_real_T *x, int32_T
  ix0, real_T y_data[1169], int32_T y_size[1], int32_T iy0);
...
static void c_eml_xaxpy(int32_T n, real_T a, const real_T x_data[1169], const
  int32_T x_size[1], int32_T ix0, emxArray_real_T *y, int32_T iy0);
static real_T c_eml_xnrm2(int32_T n, const real_T x[10], int32_T ix0);
static void c_eml_xscal(int32_T n, real_T a, real_T x[10], int32_T ix0);
  ...
  S_size[1])
{
  ...
  unnamed_idx_0 = (int16_T)kase;
  kase = unnamed_idx_0 - 1;
  for (mm = 0; mm <= kase; mm++) {
    s_data[mm] = 0.0;
  }

  memset(&e[0], 0, 10U * sizeof(real_T));
  work_size[0] = (int16_T)n;
  kase = (int16_T)n - 1;
  for (mm = 0; mm <= kase; mm++) {
    work_data[mm] = 0.0;
  }
  ...
  if (A->size[0] == 0) {
    ...
    for (q = 0; q + 1 <= mm; q++) {
      ...
      iter = n - q;
      if (q + 1 <= nct) {
        ztest0 = b_eml_xnrm2(iter, b_A, kase + 1);
        if (ztest0 > 0.0) {
          ...
          s_data[q] = ztest0;
          b_eml_xscal(iter, eml_div(1.0, s_data[q]), b_A, kase + 1);
          b_A->data[kase]++;
          s_data[q] = -s_data[q];
        } else {
          s_data[q] = 0.0;
      ...
      if (q + 1 <= nrt) {
        ...
        if (ztest0 == 0.0) {
          ...
          e[q] = ztest0;
          c_eml_xscal(9 - q, eml_div(1.0, e[q]), e, qp1);
          e[qp1 - 1]++;
        }
        ...
        if ((qp1 <= n) && (e[q] != 0.0)) {
          for (kase = qp1; kase <= n; kase++) {
            work_data[kase - 1] = 0.0;
          }

          for (qs = qp1; qs < 11; qs++) {
            b_eml_xaxpy(iter - 1, e[qs - 1], b_A, qp1 + n * (qs - 1), work_data,
                        work_size, qp1);
          }

          for (qs = qp1; qs < 11; qs++) {
            c_eml_xaxpy(iter - 1, eml_div(-e[qs - 1], e[qp1 - 1]), work_data,
                        work_size, qp1, b_A, qp1 + n * (qs - 1));
          }
        }
      }
    }
