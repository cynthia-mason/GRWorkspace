...
! * Change wp
! * Change _complex, _single
! * Change BLAS calls: caxpy, cgemm, cgemv, cpotrf, cswap, cherk, ctrsm, ctrsv,
!                      scnrm2, ctrmv, ctrmm
! * Change HSL calls: mc30a, mc77i, mc77a
! * Change control%u default to 0.1
! * Change control%small to 1e-12
!
module hsl_ma97_complex
!$ use omp_lib
   !%%use logger
   use hsl_mc34_single ! Not complex as only used for scaling and integer work
...
!$    integer(omp_lock_kind) :: lock
   ...
   type ma97_control
      ...
      integer :: scaling = 0 ! controls use of scaling. 
         ...
         !  3: mc64 from mc80
         !  >=4: mc30 
      real(wp) :: small = 1e-12_wp ! Minimum pivot size (absolute value of a
   ...
   interface ma97_analyse
      module procedure analyse_complex
   end interface ma97_analyse

   interface ma97_analyse_coord
      module procedure ma97_analyse_coord_complex
   end interface ma97_analyse_coord

   interface ma97_factor
      module procedure ma97_factor_complex
   end interface ma97_factor

   interface ma97_factor_solve
      module procedure ma97_factor_solve_complex
      module procedure ma97_factor_solve_one_complex
   end interface ma97_factor_solve

   interface ma97_solve
      module procedure ma97_solve_one_complex
      module procedure ma97_solve_mult_complex
   end interface ma97_solve

   interface ma97_solve_fredholm
      module procedure ma97_solve_fredholm_complex
   end interface ma97_solve_fredholm

   interface ma97_free
      module procedure free_akeep_complex
      module procedure free_fkeep_complex
   end interface ma97_free

   interface ma97_finalise
      module procedure finalise_both_complex
   end interface ma97_finalise

   interface ma97_enquire_posdef
      module procedure ma97_enquire_posdef_complex
   end interface ma97_enquire_posdef

   interface ma97_enquire_indef
      module procedure ma97_enquire_indef_complex
   end interface ma97_enquire_indef

   interface ma97_alter
      module procedure ma97_alter_complex
   end interface ma97_alter

   interface ma97_lmultiply
      module procedure ma97_lmultiply_one_complex
      module procedure ma97_lmultiply_mult_complex
   end interface ma97_lmultiply

   interface ma97_sparse_fwd_solve
      module procedure ma97_sparse_fwd_solve_complex
   end interface ma97_sparse_fwd_solve

   interface ma97_get_n__
      module procedure ma97_get_n_complex
   end interface ma97_get_n__

   interface ma97_get_nz__
      module procedure ma97_get_nz_complex
...
!
subroutine analyse_complex(check, n, ptr, row, akeep, control, info, &
      order, val)
...
end subroutine analyse_complex
...
! required by the factorization are set up.
!
subroutine ma97_analyse_coord_complex(n, ne, row, col, akeep, control, &
      info, order, val)
   integer, intent(in) :: n ! order of A
...
end subroutine ma97_analyse_coord_complex
...
!
subroutine ma97_factor_complex(matrix_type, val, akeep, fkeep, control, &
      info, scale, ptr, row)
   ...
   call ma97_factor_solve_complex(matrix_type, val, 0, x, 0, akeep, &
      fkeep, control, info, scale=scale, ptr=ptr, row=row)
end subroutine ma97_factor_complex

!****************************************************************************
!
! Factorize phase plus solve for single x
!
subroutine ma97_factor_solve_one_complex(matrix_type,val,x1,akeep, &
   ...
   call ma97_factor_solve_complex(matrix_type,val,1,x1,ldx1,akeep, &
      fkeep,control,info,scale=scale,ptr=ptr,row=row)
end subroutine ma97_factor_solve_one_complex

!****************************************************************************
!
! Factorize phase plus solve for multiple rhs
!
subroutine ma97_factor_solve_complex(matrix_type, val, nrhs, x, ldx, &
...
end subroutine ma97_factor_solve_complex
...
subroutine mc77_scale(n, invp, ptr, row, val, scaling, info, st)
   ...
   call mc77a(0, n, n, ne, ptr, row, val2, iw, size(iw), &
...
subroutine mc30_scale(n, invp, ptr, row, val, scaling, info, st)
   ...
   call mc30a(n, nnz, val2, row, col, scale30, work, 6, info)
...
subroutine slv_fwd_mf(pos_def, nelim, l, ldl, nrhs, x, ldx, xstack, ldxs, blas3)
   ...
   if(nrhs.eq.1 .and. .not.blas3) then
      ! Perform solve
      if(pos_def) then
         call ctrsv('Lower', 'Non-Trans', 'Non-Unit', nelim, l, ldl, x, 1)
      else
         call ctrsv('Lower', 'Non-Trans', 'Unit', nelim, l, ldl, x, 1)
      end if

      ! Update x values corresponding to delays
      if(ldx.gt.nelim) then
         call cgemv('N', ldx-nelim, nelim, -one, l(nelim+1), ldl, &
            x, 1, one, x(nelim+1), 1)
      end if
      ...
      if(ldxs.gt.0) then
         if(nelim.ne.0) then
            call cgemv('N', ldxs, nelim, -one, l(ldx+1), ldl, x, 1, &
               zero, xstack, 1)
         else
            xstack(1:ldxs) = zero
         end if
      ...
      ! Perform solve
      if(pos_def) then
         call ctrsm('Left', 'Lower', 'Non-Trans', 'Non-Unit', nelim, nrhs, &
            one, l, ldl, x, ldx)
      else
         call ctrsm('Left', 'Lower', 'Non-Trans', 'Unit', nelim, nrhs, &
            one, l, ldl, x, ldx)
      end if

      ! Update x values corresponding to delays
      if(ldx.gt.nelim) then
         call cgemm('N', 'N', ldx-nelim, nrhs, nelim, -one, l(nelim+1), ldl, &
            x, ldx, one, x(nelim+1), ldx)
      end if

      ! Update uneliminated contributions
      if(ldxs.gt.0) then
         call cgemm('N', 'N', ldxs, nrhs, nelim, -one, l(ldx+1), ldl, x, ldx, &
            zero, xstack, ldxs)
      end if
   end if
...
! In the serial case could just use cherk (but don't for bit-compatibility).
! Split as:
! ( L_1 ) ( L_1' L_2' ) + ( C_11      )
! ( L_2 )                 ( C_21 C_22 )
...
! Note: n <= m
!
subroutine mycherk(m, n, k, alpha, l, ldl, beta, c, ldc, &
      min_ldsrk_work)
...
!$OMP    SHARED(alpha, beta, c, k, l, ldc, ldl)
      ...
      ip = (i-1)*ldc + i
      call cherk('L', 'N', blkn, k, alpha, l(i), ldl, beta, c(ip), ldc)
      !%%if(dolog) call log_stop(ltask, LOG_UNIT)
!$OMP END TASK
      do j = i+blkn, m, minsz
...
!$OMP       SHARED(alpha, beta, c, k, l, ldc, ldl)
         !%%if(dolog) call log_start(ltask, "DL")
         ip = (i-1)*ldc + j
         call cgemm('N', 'C', blkm, blkn, k, cmplx(alpha,kind=wp), l(j), ldl, &
...
!$       this_thread = omp_get_thread_num()+1
         ...
         if(herm) then
            call cgemm('N', 'C', blkm, blkn, k, alpha, l(j), ldl, &
               buf(this_thread)%val, blkn, beta, c(ip), ldc)
         else
            call cgemm('N', 'T', blkm, blkn, k, alpha, l(j), ldl, &
               buf(this_thread)%val, blkn, beta, c(ip), ldc)
         end if
         !%%if(dolog) call log_stop(ltask, LOG_UNIT)
         100 continue
...
!
subroutine ma97_solve_one_complex(x, akeep, fkeep, control, info, job)
   ...
   ldx = size(x)
   call ma97_solve_mult_complex(1, x, ldx, akeep, fkeep, control, info, &
      job=job)
end subroutine ma97_solve_one_complex

!*************************************************************************

subroutine ma97_solve_mult_complex(nrhs, x, ldx, akeep, fkeep, control, &
      info, job)
   integer, intent(in) :: nrhs
   integer, intent(in) :: ldx
...
!$OMP END PARALLEL
   ...
   call ma97_print_flag(context,nout,info%flag,st=info%stat)
   return
end subroutine ma97_solve_mult_complex

!*************************************************************************
!
! Alternative solve phase (A indefinite and singular).
!
subroutine ma97_solve_fredholm_complex(nrhs, flag_out, x, ldx, &
   ...
   if (fkeep%flag.ne.MA97_WARNING_FACT_SINGULAR) then
      call ma97_solve_mult_complex(nrhs, x, ldx, akeep, fkeep, control, &
         info)
      return
   end if
   ...
   if (allocated(fkeep%scaling)) then
    ...
    do i = 1,nrhs
       tol(i) = control%consist_tol * n * scnrm2(n,x(:,i),1)
    end do
...
end subroutine ma97_solve_fredholm_complex
...
! y = Lx or y = L^Tx
!
subroutine ma97_lmultiply_one_complex(trans, x1, y1, akeep, fkeep, &
      control, info)
   ...
   ldx = size(x1)
   ldy = size(y1)
   call ma97_lmultiply_mult_complex(trans,1,x1,ldx,y1,ldy, &
      akeep,fkeep,control,info)
end subroutine ma97_lmultiply_one_complex
...
! y = Lx or y = L^Tx
!
subroutine ma97_lmultiply_mult_complex(trans, k, x, ldx, y, ldy, &
      akeep, fkeep, control, info)
   logical, intent(in) :: trans
   integer, intent(in) :: k
   integer, intent(in) :: ldx
...
end subroutine ma97_lmultiply_mult_complex
...
subroutine ma97_sparse_fwd_solve_complex(nbi, bindex, b, order, lflag, &
...
end subroutine ma97_sparse_fwd_solve_complex
...
subroutine node_mult(herm, pos_def, trans, nvec, x, ldx, y, ldy, rlist, invp, &
   ...
   if(nvec.eq.1) then ! matrix-vector version
      if(.not.trans) then
         if (blkm-nelim.gt.0) &
            call cgemv('N', blkm-nelim, nelim, one, lcol(nelim+1), blkm, &
               xlocal, 1, zero, xlocal(nelim+1), 1)
         ! dtrmv comes last as it is destructive of xlocal
         call ctrmv('L', 'N', diag, nelim, lcol, blkm, xlocal, 1)
      else
         if(herm) then
            ! dtrmv comes first as dgemv is destructive of xlocal
            call ctrmv('L', 'C', diag, nelim, lcol, blkm, xlocal, 1)
            if (blkm-nelim.gt.0) &
               call cgemv('C', blkm-nelim, nelim, one, lcol(nelim+1), blkm, &
                  xlocal(nelim+1), 1, one, xlocal, 1)
         else ! symmetric transpose
            ! dtrmv comes first as dgemv is destructive of xlocal
            call ctrmv('L', 'T', diag, nelim, lcol, blkm, xlocal, 1)
            if (blkm-nelim.gt.0) &
               call cgemv('T', blkm-nelim, nelim, one, lcol(nelim+1), blkm, &
                  xlocal(nelim+1), 1, one, xlocal, 1)
         end if
      end if
      ...
      if(.not.trans) then
         if (blkm-nelim.gt.0) &
            call cgemm('N', 'N', blkm-nelim, nvec, nelim, one, lcol(nelim+1), &
               blkm, xlocal, blkm, zero, xlocal(nelim+1), blkm)
         ! dtrmv comes last as it is destructive of xlocal
         call ctrmm('Left', 'Lower', 'Non-T', diag, nelim, nvec, one, &
            lcol, blkm, xlocal, blkm)
      else
         if(herm) then
            ! dtrmv comes first as dgemv is destructive of xlocal
            call ctrmm('Left', 'Lower', 'ConjT', diag, nelim, nvec, one, &
               lcol, blkm, xlocal, blkm)
            if (blkm-nelim.gt.0) &
               call cgemm('C', 'N', nelim, nvec, blkm-nelim, one, &
                  lcol(nelim+1), blkm, xlocal(nelim+1), blkm, one, xlocal, blkm)
         else ! not hermitian
            ! dtrmv comes first as dgemv is destructive of xlocal
            call ctrmm('Left', 'Lower', 'Trans', diag, nelim, nvec, one, &
               lcol, blkm, xlocal, blkm)
            if (blkm-nelim.gt.0) &
               call cgemm('T', 'N', nelim, nvec, blkm-nelim, one, &
                  lcol(nelim+1), blkm, xlocal(nelim+1), blkm, one, xlocal, blkm)
         endif
      end if
   end if
...
subroutine slv_fwd_one(pos_def, rlist, invp, x, blkm, blkn, nelim, nd, lcol, &
   ...
   if (blkm.gt.10 .and. nelim.gt.4) then
      ...
      if(blas3) then
         if(pos_def) then
            call ctrsm('Left', 'Lower', 'Non-Trans', 'Non-Unit', nelim, 1, &
               one, lcol, blkm, xlocal, blkm)
         else
            call ctrsm('Left', 'Lower', 'Non-Trans', 'Unit', nelim, 1, &
               one, lcol, blkm, xlocal, blkm)
         end if
      else
         if(pos_def) then
            call ctrsv('L','N','N', nelim, lcol, blkm, xlocal, 1)
         else
            call ctrsv('L','N','U', nelim, lcol, blkm, xlocal, 1)
         end if
      end if

      if (blkm-nelim.gt.0) then
         if(blas3) then
            call cgemm('N', 'N', blkm-nelim, 1, nelim, -one, &
...
subroutine slv_fwd_mult(pos_def, rlist, invp, nrhs, x, ldx, blkm, blkn, nelim, &
   ...
   if (blkm.gt.10 .and. nelim.gt.4) then
      ...
      if (blkm-nelim.gt.0) then
         call cgemm('N', 'N', blkm-nelim, nrhs, nelim, -one, &
            lcol(nelim+1), blkm, xlocal, blkm, zero, &
...
subroutine slv_bwd_one(herm, pos_def, job, rlist, invp, x, blkm, blkn, nelim, &
   ...
   if (blkm.gt.10 .and. nelim.gt.4) then
      ...
      if (blkm-nelim.gt.0) then
         if(blas3) then
            if(herm) then
               call cgemm('Conjg', 'Non-trans', nelim, 1, blkm-nelim, -one, &
                  lcol(nelim+1), blkm, xlocal(nelim+1), blkm, one, xlocal, &
                  blkm)
            else
               call cgemm('Trans', 'Non-trans', nelim, 1, blkm-nelim, -one, &
                  lcol(nelim+1), blkm, xlocal(nelim+1), blkm, one, xlocal, &
                  blkm)
            end if
         else
      ...
      if(blas3) then
         if(pos_def) then
            call ctrsm('Left','Lower','Conjg','Non-Unit', nelim, 1, one, &
               lcol, blkm, xlocal, blkm)
         elseif(herm) then
            call ctrsm('Left','Lower','Conjg','Unit', nelim, 1, one, lcol, &
               blkm, xlocal, blkm)
         else
            call ctrsm('Left','Lower','Trans','Unit', nelim, 1, one, lcol, &
               blkm, xlocal, blkm)
         end if
      else
         if(pos_def) then
            call ctrsv('L','C','N', nelim, lcol, blkm, xlocal, 1)
         elseif(herm) then
            call ctrsv('L','C','U', nelim, lcol, blkm, xlocal, 1)
         else
            call ctrsv('L','T','U', nelim, lcol, blkm, xlocal, 1)
         end if
      end if
...
subroutine slv_bwd_mult(herm, pos_def, job, rlist, invp, nrhs, x, ldx, blkm, &
   ...
   if (blkm.gt.10 .and. nelim.gt.4) then
      ...
      if (blkm-nelim.gt.0) then
         if(herm) then
            call cgemm('Conjg', 'Non-trans', nelim, nrhs, blkm-nelim, -one, &
               lcol(nelim+1), blkm, xlocal(nelim+1,1), blkm, one, xlocal, &
               blkm)
         else
            call cgemm('Trans', 'Non-trans', nelim, nrhs, blkm-nelim, -one, &
               lcol(nelim+1), blkm, xlocal(nelim+1,1), blkm, one, xlocal, &
               blkm)
         end if
      end if
      ...
      if(pos_def) then
         call ctrsm('Left','Lower','Conjg','Non-Unit', nelim, nrhs, one, lcol, &
            blkm, xlocal, blkm)
      elseif(herm) then
         call ctrsm('Left','Lower','Conjg','Unit', nelim, nrhs, one, lcol, &
            blkm, xlocal, blkm)
      else
         call ctrsm('Left','Lower','Trans','Unit', nelim, nrhs, one, lcol, &
            blkm, xlocal, blkm)
      end if
      do r = 1, nrhs
         do i = 1, nelim
...
!
subroutine ma97_enquire_posdef_complex(akeep, fkeep, control, info, d)
   type(ma97_akeep), intent(in) :: akeep
...
end subroutine ma97_enquire_posdef_complex
...
!
subroutine ma97_enquire_indef_complex(akeep, fkeep, control, info, &
      piv_order, d)
   type(ma97_akeep), intent(in) :: akeep
   type(ma97_fkeep), target, intent(in) :: fkeep
   type(ma97_control), intent(in) :: control
...
end subroutine ma97_enquire_indef_complex

!*************************************************************************
!
! In indefinite case, the entries of D^{-1} may be changed using this routine.
!
subroutine ma97_alter_complex(d, akeep, fkeep, control, info)
...
end subroutine ma97_alter_complex

!*************************************************************************

subroutine free_akeep_complex(akeep)
   type(ma97_akeep), intent(inout) :: akeep
...
end subroutine free_akeep_complex

!*******************************

subroutine free_fkeep_complex(fkeep)
   type(ma97_fkeep), intent(inout) :: fkeep
   ...
   deallocate(fkeep%nodes, stat=st)
   deallocate(fkeep%scaling, stat=st)
end subroutine free_fkeep_complex

!*******************************

subroutine finalise_both_complex(akeep, fkeep)
   type(ma97_akeep), intent(inout) :: akeep
   type(ma97_fkeep), intent(inout) :: fkeep

   call free_akeep_complex(akeep)
   call free_fkeep_complex(fkeep)
end subroutine finalise_both_complex
...
!$ this_thread = omp_get_thread_num() + 1
   ...
   if (p .le. minsz .or. maxdepth.eq.1) then
      if(pos_def) then
         call cpotrf('L', p, a, lda, lapack_info)
         if(lapack_info.ne.0) then
            stats(this_thread)%flag = MA97_ERROR_NOT_POS_DEF
            return
         end if
         call ctrsm('Right', 'Lower', 'Conjg', 'Non-unit', n-p, p, &
            one, a, lda, a(p+1), lda)
         q = p
      else
         nullify(buf(this_thread)%chkptr)
      ...
      if (q1.ge.1) then
         if(pos_def) then
            call mycherk(mu, nu, q1, -rone, a(ip), lda, &
...
subroutine factor_solve_block(herm, n, p, nb, s, a, lda, d, buf, perm,  &
   ...
   pivot: do
      ...
      sweep: do mdummy = 1, p-q ! Look for a pivotal column or pair of columns
         ...
         k = kkr1+m-rm-1
         if (q>rm) then
            !call cgemv('NoTrans',n-m+1,q-rm,-one,a(k),lda, &
            !   buf(lda*(rm+0_long)+m),lda,one,a(kkm),1)
            call mycgemv(n-m+1,q-rm,a(k),lda, &
      ...
      pivsizes: if (pivsiz.eq.1) then
         ...
         do j = q+2, min(nb,m)
            kkj = kkj + lda + 1
            call caxpy(n-j+1,-buf(kb1+j-q-1),a(kkq1+j-q-1),1,a(kkj),1)
         end do
         ! Update q and kkq1
         kkq1 = kkq1 + lda + 1
         q = q+1
         ...
         j = max(q+3,1)
         if (m>=j) then
            call cgemm('n','t',n-j+1,m-j+1,2,-one,a(kkq1+j-q-1), &
...
subroutine ma64_swap(herm,n,nb,q,q1,rm,a,lda,aleft,aln,buf,ldbuf,perm,j1,j2)
   ...
   call cswap(q-q1+1-rm,buf(j1+rm*ldbuf),ldbuf,buf(j2+rm*ldbuf),ldbuf)
   ...
   l = j1-1
   if (l>0) call cswap(l,a(k1),lda,a(k2),lda)
   if (aln>0) call cswap(aln,aleft(j1),lda,aleft(j2),lda)
   ...
   jb = j2-j1-1
   l = min(jb,nb-j1,j2-1)
   if (l>0.and.j2.ge.1) call cswap(l,a(k1),1,a(k2),lda)
   if(herm) then
...
subroutine ma64_update(n,nb,kkq,a,lda,buf,ldbuf,jl,jr,pl,pr)
   ...
   do i = 1, j2-j+1, bs
      ...
      p = min(bs, j2-j+1-i+1)
      call cgemm ('n', 't', n-j+1-i+1, p, pr-pl+1, -one, &
...
!*******************************
! Equivalent to call:
! call cgemv('NoTrans', m, n, -one, a, lda, b, ldb, one, c, 1)
! However for small updates exploit explicit loops as this is faster
! (calls cgemv itself for large updates)
subroutine mycgemv(m, n, a, lda, b, ldb, c)
   integer, intent(in) :: m
   integer, intent(in) :: n
   ...
   select case(n)
   case(1)
      !call cgemv('NoTrans', m, 1, -one, a, lda, b, ldb, one, c, 1)
      b1 = b(1)
      c(1:m) = c(1:m) - b1 * a(1:m)
   case(2)
      !call cgemv('NoTrans', m, 2, -one, a, lda, b, ldb, one, c, 1)
      b1 = b(1)
      b2 = b(1+ldb)
...
pure integer function ma97_get_n_complex(akeep)
   type(ma97_akeep), intent(in) :: akeep

   ma97_get_n_complex = akeep%n
end function ma97_get_n_complex

pure integer function ma97_get_nz_complex(akeep)
   type(ma97_akeep), intent(in) :: akeep

   ma97_get_nz_complex = akeep%ne
end function ma97_get_nz_complex

end module hsl_ma97_complex
