...
! * Change wp
! * Change _double_complex, _double
! * Change BLAS calls: zaxpy, zgemm, zgemv, zpotrf, zswap, zherk, ztrsm, ztrsv,
!                      dznrm2, ztrmv, ztrmm
! * Change HSL calls: mc30ad, mc77id, mc77ad
! * Change control%u default to 0.1
! * Change control%small to 1e-12
!
module hsl_ma97_double_complex
!$ use omp_lib
   !%%use logger
   use hsl_mc34_double ! Not complex as only used for scaling and integer work
   ...
   complex(wp), parameter :: one = 1.0_wp
   complex(wp), parameter :: zero = 0.0_wp
   ...
   type smalloc_type
      complex(wp), dimension(:), allocatable :: rmem ! real/complex memory
...
!$    integer(omp_lock_kind) :: lock
   ...
   type stack_mem_type
      complex(wp), dimension(:), allocatable :: mem ! real memory
   ...
   type node_type
      integer :: nelim
      integer :: ndelay
      complex(wp), dimension(:), pointer :: lcol ! values in factors
   ...
   type ma97_fkeep
      ...
      logical :: herm ! set to true if user indicates matrix Hermitian
   ...
   !
   type stack_type
      complex(wp), dimension(:), pointer :: val => null() ! generated element
   ...
   !
   type real_ptr_type
      complex(wp), pointer :: chkptr => null()
   ...
   type ma97_control
      ...
      !
      real(wp) :: consist_tol = epsilon(rone) ! used on call to 
   ...
   interface ma97_analyse
      module procedure analyse_double_complex
   end interface ma97_analyse

   interface ma97_analyse_coord
      module procedure ma97_analyse_coord_double_complex
   end interface ma97_analyse_coord

   interface ma97_factor
      module procedure ma97_factor_double_complex
   end interface ma97_factor

   interface ma97_factor_solve
      module procedure ma97_factor_solve_double_complex
      module procedure ma97_factor_solve_one_double_complex
   end interface ma97_factor_solve

   interface ma97_solve
      module procedure ma97_solve_one_double_complex
      module procedure ma97_solve_mult_double_complex
   end interface ma97_solve

   interface ma97_solve_fredholm
      module procedure ma97_solve_fredholm_double_complex
   end interface ma97_solve_fredholm

   interface ma97_free
      module procedure free_akeep_double_complex
      module procedure free_fkeep_double_complex
   end interface ma97_free

   interface ma97_finalise
      module procedure finalise_both_double_complex
   end interface ma97_finalise

   interface ma97_enquire_posdef
      module procedure ma97_enquire_posdef_double_complex
   end interface ma97_enquire_posdef

   interface ma97_enquire_indef
      module procedure ma97_enquire_indef_double_complex
   end interface ma97_enquire_indef

   interface ma97_alter
      module procedure ma97_alter_double_complex
   end interface ma97_alter

   interface ma97_lmultiply
      module procedure ma97_lmultiply_one_double_complex
      module procedure ma97_lmultiply_mult_double_complex
   end interface ma97_lmultiply

   interface ma97_sparse_fwd_solve
      module procedure ma97_sparse_fwd_solve_double_complex
   end interface ma97_sparse_fwd_solve

   interface ma97_get_n__
      module procedure ma97_get_n_double_complex
   end interface ma97_get_n__

   interface ma97_get_nz__
      module procedure ma97_get_nz_double_complex
...
!
subroutine analyse_double_complex(check, n, ptr, row, akeep, control, info, &
      order, val)
   ...
   complex(wp), optional, intent(in) :: val(:) ! must be present
   ...
   complex(wp), dimension(:), allocatable :: val_clean ! cleaned values if
     ! val is present and checking is required. 
   complex(wp), dimension(:), allocatable :: val2 ! expanded matrix if
     ! val is present.
   ...
   if (check) then
      ...
      if (present(val)) then
         call mc69_cscl_convert(HSL_MATRIX_CPLX_SYM, n, n, ptr, row, &
...
end subroutine analyse_double_complex
...
! required by the factorization are set up.
!
subroutine ma97_analyse_coord_double_complex(n, ne, row, col, akeep, control, &
      info, order, val)
   integer, intent(in) :: n ! order of A
   ...
   complex(wp), dimension(:), allocatable :: val_clean ! cleaned values if
     ! val is present.
   complex(wp), dimension(:), allocatable :: val2 !expanded matrix (val present)
   ...
   if (present(val)) then
      call mc69_coord_convert(HSL_MATRIX_CPLX_SYM, n, n, ne, row, col, &
         akeep%ptr, akeep%row, flag69, val_in=val, val_out=val_clean,        &
         lmap=akeep%lmap, map=akeep%map, &
         noor=info%matrix_outrange,  ndup=info%matrix_dup)
   else
...
end subroutine ma97_analyse_coord_double_complex
...
subroutine expand_matrix(n,nz,ptr,row,val,aptr,arow,aval)
   ...
   integer, intent(in)   :: ptr(n+1)
   integer, intent(in)   :: row(nz)
   complex(wp), intent(in)  :: val(nz)
   integer, intent(out)  :: aptr(n+1)
   integer, intent(out)  :: arow(2*nz)
   complex(wp), intent(out) :: aval(2*nz)
   ...
   case(5) ! Automatic choice between AMD and MeTiS (parallel)
      ...
      if(n.gt.bigN) then
         ...
         if(oxo) then
            ...
            if(st.ne.0) goto 10
            if(real(amd_nzl,kind=wp) .gt. c(3)*real(ne,kind=wp)) then
   ...
   do node = 1, nnodes
      ...
      do j = sptr(node), sptr(node+1)-1
         ...
         do i = ptr2(col), ptr2(col+1)-1
            ...
            ! Note: use minus to indicate need to take conjugate if hermitian
            nlist(1,p) = -origin(i)
            p = p + 1
         end do
      end do
...
!
subroutine ma97_factor_double_complex(matrix_type, val, akeep, fkeep, control, &
      info, scale, ptr, row)
   ...
   call ma97_factor_solve_double_complex(matrix_type, val, 0, x, 0, akeep, &
      fkeep, control, info, scale=scale, ptr=ptr, row=row)
end subroutine ma97_factor_double_complex

!****************************************************************************
!
! Factorize phase plus solve for single x
!
subroutine ma97_factor_solve_one_double_complex(matrix_type,val,x1,akeep, &
   ...
   call ma97_factor_solve_double_complex(matrix_type,val,1,x1,ldx1,akeep, &
      fkeep,control,info,scale=scale,ptr=ptr,row=row)
end subroutine ma97_factor_solve_one_double_complex

!****************************************************************************
!
! Factorize phase plus solve for multiple rhs
!
subroutine ma97_factor_solve_double_complex(matrix_type, val, nrhs, x, ldx, &
   ...
   complex(wp), intent(inout) :: x(ldx,nrhs) ! if nrhs > 0
      ! must hold right-hand sides on entry. On exit, holds solution.
   type(ma97_akeep), intent(in) :: akeep
   type(ma97_fkeep), intent(inout) :: fkeep
   type(ma97_control), intent(in) :: control
   ...
   complex(wp), dimension(:), allocatable :: val2
   ...
   if (control%print_level >= 1 .and. mp >= 0) then
      if (matrix_type == HSL_MATRIX_CPLX_HERM_PSDEF) then
         write (mp,'(//a,i2,a)') &
   ...
   case(HSL_MATRIX_CPLX_HERM_PSDEF)
      fkeep%pos_def = .true.
      fkeep%herm = .true.
   ...
   if(associated(fkeep%alloc)) then
      ...
      if(fkeep%alloc%rmem_size.lt. max(akeep%nfactor+2*n, &
            int(control%multiplier*real(akeep%nfactor,wp)+2*n,kind=long))) then
         deallocate(fkeep%alloc%rmem, stat=st)
         fkeep%alloc%rmem_size = max(akeep%nfactor+2*n, &
            int(control%multiplier*real(akeep%nfactor,wp)+2*n,kind=long))
      ...
      call smalloc_setup(fkeep%alloc, &
         ...
         max(akeep%nfactor+2*n, &
            int(control%multiplier*real(akeep%nfactor,wp)+2*n,kind=long)), st)
      if (st .ne. 0) go to 10
   end if
...
!$OMP SINGLE
      if (akeep%check) then
         if (allocated(fkeep%scaling)) then
            call inner_factor(1, akeep%nnodes+1, fkeep%herm, fkeep%pos_def, n, &
...
!$OMP END PARALLEL
      ...
      if (akeep%check) then
         if (allocated(fkeep%scaling)) then
            call subtree_factor(1, akeep%nnodes, fkeep%herm, fkeep%pos_def,   &
...
!$OMP SINGLE
      call bwd_slv_tasks(nchunk+1, chunk_sa, chunk_en, fwd_ptr, fwd, &
         local_job, fkeep%herm, fkeep%pos_def, akeep%nnodes, fkeep%nodes, &
...
end subroutine ma97_factor_solve_double_complex
...
subroutine mc64_scale(n, ptr, row, val, scaling, invp, st, action, sing)
   ...
   complex(wp), intent(in) :: val(*) ! entries of A (in same order as in row).
...
subroutine mc30_scale(n, invp, ptr, row, val, scaling, info, st)
   ...
   real(wp), allocatable, dimension(:) :: work, scale30, val2

   info = 0
   st = 0

   nnz = ptr(n+1)-1
   allocate(col(nnz), val2(nnz), stat=st)
   if (st .ne. 0) then
      info = MA97_ERROR_ALLOCATION
      return
   end if
   ...
   val2(1:nnz) = abs(val(1:nnz)) ! convert from complex to real
   ...
   call mc30ad(n, nnz, val2, row, col, scale30, work, 6, info)
...
! nodes sa:en.
!
recursive subroutine inner_factor(sa, en, herm, pos_def, n, nnodes, child_ptr, &
      child_list, subtree_work, invp, nptr, nlist, val, &
      nodes, sptr, sparent, level, rptr, rlist, alloc2, stack, map, buf, &
   ...
   logical, intent(in) :: herm ! True if problem is Hermitian
   ...
   type(smalloc_type), target, intent(inout) :: alloc2 ! Contains actual
   ...
   complex(wp), dimension(ldx, *), intent(inout) :: x
   type(ma97_control), intent(in) :: control
...
!%%!$OMP    PRIVATE(ltask)                                                     &
!$OMP    PRIVATE(this_thread)                                                  &
!$OMP    SHARED(abort, alloc2, buf, child_list, child_ptr, control, herm, invp,&
...
!$       this_thread = omp_get_thread_num()+1
         if(.not.abort .and. (myrecursed.ge.maxrecurse .or. &
            ...
            call subtree_factor(current_start, current_end, herm, pos_def,    &
               child_ptr, child_list, n, invp, nptr, nlist, val, nnodes,      &
               nodes, sptr, sparent, level, rptr, rlist, alloc2, stack,       &
            ...
            call inner_factor(current_start, current_end, herm, pos_def, n,   &
               nnodes, child_ptr, child_list, subtree_work, invp, nptr, nlist,&
               val, nodes, sptr, sparent, level, rptr, rlist, alloc2, stack,  &
...
!$    this_thread = omp_get_thread_num()+1
      if(.not. abort .and. ( myrecursed.ge.maxrecurse .or. &
         ...
         call subtree_factor(current_start, current_end, herm, pos_def,       &
            ...
            sptr, sparent, level, rptr, rlist, alloc2, stack,                 &
            map(:,this_thread), num_threads, buf, nrhs, x, ldx, control,      &
            stats, myntask, scale=scale)
         !%%if(dolog .and. myntask.gt.1) call log_stop(ltask, LOG_UNIT)
         if(stats(this_thread)%flag.lt.0) abort = .true.
         ...
         call inner_factor(current_start, current_end, herm, pos_def, n,      &
            nnodes, child_ptr, child_list, subtree_work, invp, nptr, nlist,   &
            val, nodes, sptr, sparent, level, rptr, rlist, alloc2, stack, map,&
...
!!$ this_thread = omp_get_thread_num()+1
   ...
   if(node.le.nnodes) then
      ...
      call subtree_factor(node, node, herm, pos_def, child_ptr, child_list, &
         n, invp, nptr, nlist, val, nnodes, nodes, sptr, sparent, level, &
         rptr, rlist, alloc2, stack, map(:,this_thread), num_threads, buf, &
...
! eliminated.
!
subroutine subtree_factor(sa, en, herm, pos_def, child_ptr, child_list, n, &
   ...
   integer :: node
   complex(wp), dimension(:), pointer :: lcol
   complex(wp), dimension(:), pointer :: lstack
...
!$ this_thread = omp_get_thread_num() + 1
   ...
   do node = sa, en
      ...
      ! Add A
      if(herm) then ! Hermitian, take conjugates as approriate
         if (present(scale)) then
            do i = nptr(node), nptr(node+1)-1
               ...
               ! Take complex conjugate if nlist(1,i) is negative
               p = nlist(1,i)
               q = abs(p)
               if(j.eq.k .and. aimag(val(q)).ne.0) then
                  ! non-zero imaginary component on diagonal of Hermitian
                  ! matrix is an error.
                  stats(this_thread)%flag = MA97_ERROR_IMAG_DIAGONAL
                  return
               end if
               v = cmplx( real(val(q)), sign(1,p)*aimag(val(q)), kind=wp)
               lcol(ip) = scale(j) * v * scale(k)
            end do
         else
            do i = nptr(node), nptr(node+1)-1
               p = nlist(1,i)
               ...
               ! Take complex conjugate if nlist(1,i) is negative
               lcol(ip) = cmplx( real(val(q)), sign(1,p)*aimag(val(q)), kind=wp)
            end do
         end if
      else ! complex symmetric, no need for conjugation
         if (present(scale)) then
            do i = nptr(node), nptr(node+1)-1
               j = nlist(2,i)
               k = (j-1) / (blkm-ndelay) + 1 ! local column
               ...
               j = rlist( rptr(node)+j-1 )
               k = rlist( rptr(node)+k-1 )
               lcol(ip) = scale(j) * val(abs(nlist(1,i))) * scale(k)
            end do
         else
            do i = nptr(node), nptr(node+1)-1
               j = nlist(2,i)
               kk = (j-1) / (blkm-ndelay) ! column
               kk = (ndelay+kk)*blkm + mod(j-1, blkm-ndelay)+1 + ndelay
               lcol(kk) = val(abs(nlist(1,i)))
            end do
         end if
      end if
      ...
      call rfact_block(herm, pos_def, blkm, blkn, blkn, ndelay, &
         !lcol(1:blkm*blkn), &
         nodes(node)%rsmptr%rmem(nodes(node)%rsmsa), &
         blkm, &
         !lcol(blkm*blkn+1:), &
         nodes(node)%rsmptr%rmem(nodes(node)%rsmsa+(blkm+0_long)*blkn), &
         buf, &
         !lperm, &
      ...
      if(stats(this_thread)%flag.lt.0) then
         call stack_free_all(stack_mem_odd)
         call stack_free_all(stack_mem_even)
         return
      endif
...
subroutine slv_fwd_mf(pos_def, nelim, l, ldl, nrhs, x, ldx, xstack, ldxs, blas3)
   ...
   complex(wp), dimension(*), intent(in) :: l
   integer, intent(in) :: ldl
   integer, intent(in) :: nrhs
   complex(wp), dimension(*), intent(inout) :: x
   integer, intent(in) :: ldx
   complex(wp), dimension(*), intent(out) :: xstack
   ...
   if(nrhs.eq.1 .and. .not.blas3) then
      ! Perform solve
      if(pos_def) then
         call ztrsv('Lower', 'Non-Trans', 'Non-Unit', nelim, l, ldl, x, 1)
      else
         call ztrsv('Lower', 'Non-Trans', 'Unit', nelim, l, ldl, x, 1)
      end if

      ! Update x values corresponding to delays
      if(ldx.gt.nelim) then
         call zgemv('N', ldx-nelim, nelim, -one, l(nelim+1), ldl, &
            x, 1, one, x(nelim+1), 1)
      end if
      ...
      if(ldxs.gt.0) then
         if(nelim.ne.0) then
            call zgemv('N', ldxs, nelim, -one, l(ldx+1), ldl, x, 1, &
               zero, xstack, 1)
         else
            xstack(1:ldxs) = zero
         end if
      ...
      ! Perform solve
      if(pos_def) then
         call ztrsm('Left', 'Lower', 'Non-Trans', 'Non-Unit', nelim, nrhs, &
            one, l, ldl, x, ldx)
      else
         call ztrsm('Left', 'Lower', 'Non-Trans', 'Unit', nelim, nrhs, &
            one, l, ldl, x, ldx)
      end if

      ! Update x values corresponding to delays
      if(ldx.gt.nelim) then
         call zgemm('N', 'N', ldx-nelim, nrhs, nelim, -one, l(nelim+1), ldl, &
            x, ldx, one, x(nelim+1), ldx)
      end if

      ! Update uneliminated contributions
      if(ldxs.gt.0) then
         call zgemm('N', 'N', ldxs, nrhs, nelim, -one, l(ldx+1), ldl, x, ldx, &
            zero, xstack, ldxs)
      end if
   end if
...
! In the serial case could just use zherk (but don't for bit-compatibility).
! Split as:
! ( L_1 ) ( L_1' L_2' ) + ( C_11      )
! ( L_2 )                 ( C_21 C_22 )
...
! Note: n <= m
!
subroutine myzherk(m, n, k, alpha, l, ldl, beta, c, ldc, &
      min_ldsrk_work)
...
!$OMP    SHARED(alpha, beta, c, k, l, ldc, ldl)
      ...
      ip = (i-1)*ldc + i
      call zherk('L', 'N', blkn, k, alpha, l(i), ldl, beta, c(ip), ldc)
      !%%if(dolog) call log_stop(ltask, LOG_UNIT)
!$OMP END TASK
      do j = i+blkn, m, minsz
...
!$OMP       SHARED(alpha, beta, c, k, l, ldc, ldl)
         !%%if(dolog) call log_start(ltask, "DL")
         ip = (i-1)*ldc + j
         call zgemm('N', 'C', blkm, blkn, k, cmplx(alpha,kind=wp), l(j), ldl, &
           l(i), ldl, cmplx(beta,kind=wp), c(ip), ldc)
         !%%if(dolog) call log_stop(ltask, LOG_UNIT)
!$OMP    END TASK
      end do
   end do
!$OMP TASKWAIT
end subroutine myzherk
...
! Note: n <= m
!
subroutine ldsrk(herm, m, n, k, alpha, l, ldl, d, beta, c, ldc, buf, &
      min_ldsrk_work, stats)
   logical, intent(in) :: herm
   integer, intent(in) :: m ! number of rows in c
   integer, intent(in) :: n ! number of cols in c
   integer, intent(in) :: k ! number of cols in l and ld
   complex(wp), intent(in) :: alpha
   complex(wp), dimension(*), target, intent(in) :: l
   integer, intent(in) :: ldl
...
!%%!$OMP       PRIVATE(ltask) &
!$OMP       SHARED(buf, c, d, herm, l, stats)
         this_thread = 1
!$       this_thread = omp_get_thread_num()+1
         ...
         if(.not.associated(buf(this_thread)%chkptr, target=l(i))) then
            call calc_ld(herm, blkn, k, ldl, l(i), d, buf(this_thread)%val, &
               blkn)
            buf(this_thread)%chkptr => l(i)
         end if
         if(herm) then
            call zgemm('N', 'C', blkm, blkn, k, alpha, l(j), ldl, &
               buf(this_thread)%val, blkn, beta, c(ip), ldc)
         else
            call zgemm('N', 'T', blkm, blkn, k, alpha, l(j), ldl, &
               buf(this_thread)%val, blkn, beta, c(ip), ldc)
         end if
         !%%if(dolog) call log_stop(ltask, LOG_UNIT)
         100 continue
...
!
subroutine calc_ld(herm, m, n, ldl, l_mat, d_vec, ld_mat, ldld)
   logical, intent(in) :: herm
   ...
   complex(wp), dimension(n*ldl), intent(in) :: l_mat
   complex(wp), dimension(2*n), intent(in) :: d_vec
   integer, intent(in) :: ldld
   complex(wp), dimension(n*ldld), intent(out) :: ld_mat
      ...
      if(d_vec(2*col).eq.zero) then 
         ...
         if(herm) then
            do row = 1, m
               ...
               ld_mat(ldptr+ldld) = -conjg(d2)*l_mat(lptr) + d1*l_mat(lptr+ldl)
...
!
subroutine ma97_solve_one_double_complex(x, akeep, fkeep, control, info, job)
   complex(wp), dimension(:), intent(inout) :: x ! On entry, x must
   ...
   ldx = size(x)
   call ma97_solve_mult_double_complex(1, x, ldx, akeep, fkeep, control, info, &
      job=job)
end subroutine ma97_solve_one_double_complex

!*************************************************************************

subroutine ma97_solve_mult_double_complex(nrhs, x, ldx, akeep, fkeep, control, &
      info, job)
   integer, intent(in) :: nrhs
   integer, intent(in) :: ldx
   complex(wp), dimension(ldx,nrhs), intent(inout) :: x
...
!$OMP    END PARALLEL
   ...
   call inner_solve(fkeep%herm, fkeep%pos_def, local_job, akeep%nnodes, &
...
!$OMP END PARALLEL
   ...
   call ma97_print_flag(context,nout,info%flag,st=info%stat)
   return
end subroutine ma97_solve_mult_double_complex

!*************************************************************************
!
! Alternative solve phase (A indefinite and singular).
!
subroutine ma97_solve_fredholm_double_complex(nrhs, flag_out, x, ldx, &
      akeep, fkeep, control, info)
   integer, intent(in) :: nrhs
   logical, intent(out) :: flag_out(nrhs)
   integer, intent(in) :: ldx
   complex(wp), dimension(ldx,2*nrhs), intent(inout) :: x ! On entry, x must
      ! be set so that if i has been used to index a variable,
      ! x(i,j) is the corresponding component of the
      ! right-hand side for the jth system (j = 1,2,..., nrhs).
   ...
   if (fkeep%flag.ne.MA97_WARNING_FACT_SINGULAR) then
      call ma97_solve_mult_double_complex(nrhs, x, ldx, akeep, fkeep, control, &
         info)
      return
   end if
   ...
   if (allocated(fkeep%scaling)) then
    ...
    do i = 1,nrhs
       tol(i) = control%consist_tol * n * dznrm2(n,x(:,i),1)
    end do
...
!$OMP    END PARALLEL
   ...
   call diag_solve(fkeep%herm, n, akeep%nnodes, fkeep%nodes, akeep%sptr, &
...
!$OMP SINGLE
   ...
   call bwd_slv_tasks(nchunk+1, chunk_sa, chunk_en, fwd_ptr, fwd,    &
      MA97_SOLVE_JOB_BWD, fkeep%herm, fkeep%pos_def, akeep%nnodes,   &
...
end subroutine ma97_solve_fredholm_double_complex
...
! y = Lx or y = L^Tx
!
subroutine ma97_lmultiply_one_double_complex(trans, x1, y1, akeep, fkeep, &
      control, info)
   logical, intent(in) :: trans
   complex(wp), dimension(:), intent(in) :: x1
   complex(wp), dimension(:), intent(out) :: y1
   ! For details of keep, control, info : see derived type description
   type(ma97_akeep), intent(in) :: akeep
   type(ma97_fkeep), intent(in) :: fkeep
   type(ma97_control), intent(in) :: control
   ...
   ldx = size(x1)
   ldy = size(y1)
   call ma97_lmultiply_mult_double_complex(trans,1,x1,ldx,y1,ldy, &
      akeep,fkeep,control,info)
end subroutine ma97_lmultiply_one_double_complex
...
! y = Lx or y = L^Tx
!
subroutine ma97_lmultiply_mult_double_complex(trans, k, x, ldx, y, ldy, &
      akeep, fkeep, control, info)
   logical, intent(in) :: trans
   integer, intent(in) :: k
   integer, intent(in) :: ldx
   complex(wp), dimension(ldx,k), intent(in) :: x
   integer, intent(in) :: ldy
   complex(wp), dimension(ldy,k), intent(out) :: y
   ! For details of keep, control, info : see derived type description
   type(ma97_akeep), intent(in) :: akeep
   type(ma97_fkeep), intent(in) :: fkeep
   type(ma97_control), intent(in) :: control
   ...
   do node = 1, akeep%nnodes
      ...
      if(allocated(x2)) then
         call node_mult(fkeep%herm, fkeep%pos_def, trans, k, x2, akeep%n, &
         ...
         call node_mult(fkeep%herm, fkeep%pos_def, trans, k, x, ldx, y, ldy, &
...
end subroutine ma97_lmultiply_mult_double_complex
...
subroutine ma97_sparse_fwd_solve_double_complex(nbi, bindex, b, order, lflag, &
   ...
   complex(wp), intent(in) :: b(:) ! If bindex(i)=k, b(k) must hold the k-th
   ...
   complex(wp), intent(inout) :: x(:) ! User must set to zero on entry.
...
end subroutine ma97_sparse_fwd_solve_double_complex
...
recursive subroutine bwd_slv_tasks(chunk, chunk_sa, chunk_en, fwd_ptr, fwd, &
      job, herm, pos_def, nnodes, nodes, sptr, rptr, rlist, invp, nrhs, x, &
      ldx, blas3, st)
   ...
   if(nrhs.eq.1) then
      call subtree_bwd_slv_one(chunk_en(chunk), chunk_sa(chunk), job, herm, &
         pos_def, nnodes, nodes, sptr, rptr, rlist, invp, x, ldx, blas3, st)
   else
      call subtree_bwd_slv_mult(chunk_en(chunk), chunk_sa(chunk), job, herm, &
         pos_def, nnodes, nodes, sptr, rptr, rlist, invp, nrhs, x, ldx, st)
   endif
   if(st.ne.0) return
...
! by sa:en.
!
! Note: this subtree_bwd_slv_one() and subtree_bwd_slv_mult() have been split
! as a workaround for a bug in ifort with the combination of -openmp -ip
!
subroutine subtree_bwd_slv_one(en, sa, job, herm, pos_def, nnodes, nodes, sptr,&
      rptr, rlist, invp, x, ldx, blas3, st)
   integer, intent(in) :: en
   integer, intent(in) :: sa
   logical, intent(in) :: herm
   logical, intent(in) :: pos_def
...
!  print *, "SUBTREE ", en, sa, " on ", omp_get_thread_num()
   ...
   do node = en, sa, -1
      ...
      call slv_bwd_one(herm, pos_def, job, rlist(rptr(node)), invp, x, &
         blkm, blkn, nelim, nd, &
...
end subroutine subtree_bwd_slv_one
subroutine subtree_bwd_slv_mult(en, sa, job, herm, pos_def, nnodes, nodes, &
      sptr, rptr, rlist, invp, nrhs, x, ldx, st)
   integer, intent(in) :: en
   integer, intent(in) :: sa
   logical, intent(in) :: herm
   logical, intent(in) :: pos_def
...
!  print *, "SUBTREE ", en, sa, " on ", omp_get_thread_num()
   ...
   do node = en, sa, -1
      ...
      call slv_bwd_mult(herm, pos_def, job, rlist(rptr(node)), invp, &
...
end subroutine subtree_bwd_slv_mult

!*************************************************************************
!
! Provides serial versions of Forward (s/n) and diagonal solves.
!
subroutine inner_solve(herm, pos_def, job, nnodes, nodes, sptr, rptr, rlist, &
...
!   end if
   ...
   if (job.eq.MA97_SOLVE_JOB_DIAG) then
      ...
      do node = nnodes, 1, -1
         ...
         if(nrhs.eq.1) then
            call slv_diag_one(herm, invp, x, nelim, &
               nodes(node)%rsmptr%rmem(nodes(node)%rsmsa+blkm*blkn), & ! node%d
               nodes(node)%ismptr%imem(nodes(node)%ismsa)) ! node%perm
         else
            call slv_diag_mult(herm, invp, nrhs, x, ldx, nelim, &
               nodes(node)%rsmptr%rmem(nodes(node)%rsmsa+blkm*blkn), & ! node%d
               nodes(node)%ismptr%imem(nodes(node)%ismsa)) ! node%perm
         end if
      end do
...
subroutine inner_solve_sparse(pos_def, nnodes, nodes, sptr, rptr, rlist, &
   ...
   integer, dimension(*), intent(in) :: invp
   integer, intent(in) :: n
   complex(wp), dimension(n), intent(inout) :: x
...
!
subroutine diag_solve(herm, n, nnodes, nodes, sptr, rptr, invp, nrhs, x, ldx, &
   ...
   integer, intent(in) :: nrhs
   integer, intent(in) :: ldx
   complex(wp), dimension(ldx,2*nrhs), intent(inout) :: x
   ...
   do node = 1,nnodes
      ...
      call check_diag(herm, n, invp, nrhs, x, ldx, nelim, &
         nodes(node)%rsmptr%rmem(nodes(node)%rsmsa+blkm*blkn), & ! node%d
         nodes(node)%ismptr%imem(nodes(node)%ismsa), & ! node%perm
...
!
subroutine node_mult(herm, pos_def, trans, nvec, x, ldx, y, ldy, rlist, invp, &
   ...
   integer, intent(in) :: ldx
   complex(wp), dimension(*), intent(inout) :: y
   integer, intent(in) :: ldy
   integer, dimension(*), intent(in) :: rlist
   integer, dimension(*), intent(in) :: invp
   ...
   complex(wp), dimension(*), intent(in) :: lcol
   ...
   if(nvec.eq.1) then ! matrix-vector version
      if(.not.trans) then
         if (blkm-nelim.gt.0) &
            call zgemv('N', blkm-nelim, nelim, one, lcol(nelim+1), blkm, &
               xlocal, 1, zero, xlocal(nelim+1), 1)
         ! dtrmv comes last as it is destructive of xlocal
         call ztrmv('L', 'N', diag, nelim, lcol, blkm, xlocal, 1)
      else
         if(herm) then
            ! dtrmv comes first as dgemv is destructive of xlocal
            call ztrmv('L', 'C', diag, nelim, lcol, blkm, xlocal, 1)
            if (blkm-nelim.gt.0) &
               call zgemv('C', blkm-nelim, nelim, one, lcol(nelim+1), blkm, &
                  xlocal(nelim+1), 1, one, xlocal, 1)
         else ! symmetric transpose
            ! dtrmv comes first as dgemv is destructive of xlocal
            call ztrmv('L', 'T', diag, nelim, lcol, blkm, xlocal, 1)
            if (blkm-nelim.gt.0) &
               call zgemv('T', blkm-nelim, nelim, one, lcol(nelim+1), blkm, &
                  xlocal(nelim+1), 1, one, xlocal, 1)
         end if
      end if
      ...
      if(.not.trans) then
         if (blkm-nelim.gt.0) &
            call zgemm('N', 'N', blkm-nelim, nvec, nelim, one, lcol(nelim+1), &
               blkm, xlocal, blkm, zero, xlocal(nelim+1), blkm)
         ! dtrmv comes last as it is destructive of xlocal
         call ztrmm('Left', 'Lower', 'Non-T', diag, nelim, nvec, one, &
            lcol, blkm, xlocal, blkm)
      else
         if(herm) then
            ! dtrmv comes first as dgemv is destructive of xlocal
            call ztrmm('Left', 'Lower', 'ConjT', diag, nelim, nvec, one, &
               lcol, blkm, xlocal, blkm)
            if (blkm-nelim.gt.0) &
               call zgemm('C', 'N', nelim, nvec, blkm-nelim, one, &
                  lcol(nelim+1), blkm, xlocal(nelim+1), blkm, one, xlocal, blkm)
         else ! not hermitian
            ! dtrmv comes first as dgemv is destructive of xlocal
            call ztrmm('Left', 'Lower', 'Trans', diag, nelim, nvec, one, &
               lcol, blkm, xlocal, blkm)
            if (blkm-nelim.gt.0) &
               call zgemm('T', 'N', nelim, nvec, blkm-nelim, one, &
                  lcol(nelim+1), blkm, xlocal(nelim+1), blkm, one, xlocal, blkm)
         endif
      end if
   end if
...
subroutine slv_fwd_one(pos_def, rlist, invp, x, blkm, blkn, nelim, nd, lcol, &
   ...
   if (blkm.gt.10 .and. nelim.gt.4) then
      ...
      if(blas3) then
         if(pos_def) then
            call ztrsm('Left', 'Lower', 'Non-Trans', 'Non-Unit', nelim, 1, &
               one, lcol, blkm, xlocal, blkm)
         else
            call ztrsm('Left', 'Lower', 'Non-Trans', 'Unit', nelim, 1, &
               one, lcol, blkm, xlocal, blkm)
         end if
      else
         if(pos_def) then
            call ztrsv('L','N','N', nelim, lcol, blkm, xlocal, 1)
         else
            call ztrsv('L','N','U', nelim, lcol, blkm, xlocal, 1)
         end if
      end if

      if (blkm-nelim.gt.0) then
         if(blas3) then
            call zgemm('N', 'N', blkm-nelim, 1, nelim, -one, &
...
subroutine slv_fwd_mult(pos_def, rlist, invp, nrhs, x, ldx, blkm, blkn, nelim, &
   ...
   complex(wp), dimension(*), intent(in) :: lcol
   integer, dimension(*), intent(in) :: lperm
   complex(wp), dimension(blkm,*), intent(out) :: xlocal
   integer, dimension(*), intent(out) :: map
   ...
   if (blkm.gt.10 .and. nelim.gt.4) then
      ...
      if (blkm-nelim.gt.0) then
         call zgemm('N', 'N', blkm-nelim, nrhs, nelim, -one, &
            lcol(nelim+1), blkm, xlocal, blkm, zero, &
...
!
subroutine slv_bwd_one(herm, pos_def, job, rlist, invp, x, blkm, blkn, nelim, &
   ...
   integer, intent(in) :: nd
   complex(wp), dimension(*), intent(in) :: lcol
   complex(wp), dimension(2*nelim) :: d
   integer, dimension(*), intent(in) :: lperm
   complex(wp), dimension(*), intent(out) :: xlocal
   integer, dimension(*), intent(out) :: map
   logical, intent(in) :: blas3
   ...
   if (blkm.gt.10 .and. nelim.gt.4) then
      ...
      if (blkm-nelim.gt.0) then
         if(blas3) then
            if(herm) then
               call zgemm('Conjg', 'Non-trans', nelim, 1, blkm-nelim, -one, &
                  lcol(nelim+1), blkm, xlocal(nelim+1), blkm, one, xlocal, &
                  blkm)
            else
               call zgemm('Trans', 'Non-trans', nelim, 1, blkm-nelim, -one, &
                  lcol(nelim+1), blkm, xlocal(nelim+1), blkm, one, xlocal, &
                  blkm)
            end if
         else
      ...
      if(blas3) then
         if(pos_def) then
            call ztrsm('Left','Lower','Conjg','Non-Unit', nelim, 1, one, &
               lcol, blkm, xlocal, blkm)
         elseif(herm) then
            call ztrsm('Left','Lower','Conjg','Unit', nelim, 1, one, lcol, &
               blkm, xlocal, blkm)
         else
            call ztrsm('Left','Lower','Trans','Unit', nelim, 1, one, lcol, &
               blkm, xlocal, blkm)
         end if
      else
         if(pos_def) then
            call ztrsv('L','C','N', nelim, lcol, blkm, xlocal, 1)
         elseif(herm) then
            call ztrsv('L','C','U', nelim, lcol, blkm, xlocal, 1)
         else
            call ztrsv('L','T','U', nelim, lcol, blkm, xlocal, 1)
         end if
      end if
      ...
      if(herm) then
         do i = 1, nelim
            ...
            do j = nelim+1, blkm
               rp1 = map(j)
               xlocal(i) = xlocal(i) - x(rp1) * conjg(lcol(ip+j))
            end do
         end do
      else
         do i = 1, nelim
      ...
      if(pos_def) then
         do i = nelim, 1, -1
            ...
            rp1 = map(i)
            xlocal(i) = xlocal(i) - &
               sum(xlocal(i+1:nelim) * conjg(lcol(ip+i+1:ip+nelim)))
...
!
subroutine slv_bwd_mult(herm, pos_def, job, rlist, invp, nrhs, x, ldx, blkm, &
   ...
   if (blkm.gt.10 .and. nelim.gt.4) then
      ...
      if (blkm-nelim.gt.0) then
         if(herm) then
            call zgemm('Conjg', 'Non-trans', nelim, nrhs, blkm-nelim, -one, &
               lcol(nelim+1), blkm, xlocal(nelim+1,1), blkm, one, xlocal, &
               blkm)
         else
            call zgemm('Trans', 'Non-trans', nelim, nrhs, blkm-nelim, -one, &
               lcol(nelim+1), blkm, xlocal(nelim+1,1), blkm, one, xlocal, &
               blkm)
         end if
      end if
      ...
      if(pos_def) then
         call ztrsm('Left','Lower','Conjg','Non-Unit', nelim, nrhs, one, lcol, &
            blkm, xlocal, blkm)
      elseif(herm) then
         call ztrsm('Left','Lower','Conjg','Unit', nelim, nrhs, one, lcol, &
            blkm, xlocal, blkm)
      else
         call ztrsm('Left','Lower','Trans','Unit', nelim, nrhs, one, lcol, &
            blkm, xlocal, blkm)
      end if
      do r = 1, nrhs
         do i = 1, nelim
      ...
      do r = 1, nrhs
         if(herm) then
            do i = 1, nelim
               ...
               do j = nelim+1, blkm
                  rp1 = map(j)
                  xlocal(i,r) = xlocal(i,r) - x(rp1,r) * conjg(lcol(ip+j))
               end do
            end do
         else
            do i = 1, nelim
         ...
         if(pos_def) then
            do i = nelim, 1, -1
               ...
               xlocal(i,r) = xlocal(i,r) - &
                  sum(xlocal(i+1:nelim,r) * conjg(lcol(ip+i+1:ip+nelim)))
...
!
subroutine slv_diag_one(herm, invp, x, nelim, d, lperm)
   logical, intent(in) :: herm
   integer, dimension(*), intent(in) :: invp
   complex(wp), dimension(*), intent(inout) :: x
   integer, intent(in) :: nelim
...
!
subroutine slv_diag_mult(herm, invp, nrhs, x, ldx, nelim, d, lperm)
   logical, intent(in) :: herm
   integer, dimension(*), intent(in) :: invp
   integer, intent(in) :: nrhs
   integer, intent(in) :: ldx
...
!
subroutine check_diag(herm, n, invp, nrhs, x, ldx, nelim, d, lperm, &
      flag_out, num_false, tol)
   logical, intent(in) :: herm
   integer, intent(in) :: n
   integer, dimension(*), intent(in) :: invp
...
!
subroutine ma97_enquire_posdef_double_complex(akeep, fkeep, control, info, d)
   type(ma97_akeep), intent(in) :: akeep
   ...
   d(1:n) = rzero ! ensure do not returned with this undefined
   ...
   do node = 1, akeep%nnodes
      ...
      i = 1
      do j = 1, blkn
         d(piv) = real(nptr%lcol(i))
         i = i + blkm + 1
         piv = piv + 1
      end do
   end do
end subroutine ma97_enquire_posdef_double_complex
...
!
subroutine ma97_enquire_indef_double_complex(akeep, fkeep, control, info, &
      piv_order, d)
   type(ma97_akeep), intent(in) :: akeep
   type(ma97_fkeep), target, intent(in) :: fkeep
   type(ma97_control), intent(in) :: control
   ...
   complex(wp), dimension(2,*), optional, intent(out) :: d ! The diagonal
...
end subroutine ma97_enquire_indef_double_complex

!*************************************************************************
!
! In indefinite case, the entries of D^{-1} may be changed using this routine.
!
subroutine ma97_alter_double_complex(d, akeep, fkeep, control, info)
   complex(wp), dimension(2,*), intent(in) :: d  ! The required diagonal entries
...
end subroutine ma97_alter_double_complex

!*************************************************************************

subroutine free_akeep_double_complex(akeep)
   type(ma97_akeep), intent(inout) :: akeep
...
end subroutine free_akeep_double_complex

!*******************************

subroutine free_fkeep_double_complex(fkeep)
   type(ma97_fkeep), intent(inout) :: fkeep
   ...
   deallocate(fkeep%nodes, stat=st)
   deallocate(fkeep%scaling, stat=st)
end subroutine free_fkeep_double_complex

!*******************************

subroutine finalise_both_double_complex(akeep, fkeep)
   type(ma97_akeep), intent(inout) :: akeep
   type(ma97_fkeep), intent(inout) :: fkeep

   call free_akeep_double_complex(akeep)
   call free_fkeep_double_complex(fkeep)
end subroutine finalise_both_double_complex
...
recursive subroutine rfact_block(herm, pos_def, n, p, nb, s, a, lda, d, buf, &
...
!$ this_thread = omp_get_thread_num() + 1
   ...
   if (p .le. minsz .or. maxdepth.eq.1) then
      if(pos_def) then
         call zpotrf('L', p, a, lda, lapack_info)
         if(lapack_info.ne.0) then
            stats(this_thread)%flag = MA97_ERROR_NOT_POS_DEF
            return
         end if
         call ztrsm('Right', 'Lower', 'Conjg', 'Non-unit', n-p, p, &
            one, a, lda, a(p+1), lda)
         q = p
      else
         nullify(buf(this_thread)%chkptr)
         ...
         call factor_solve_block(herm, n, p, nb, s, a, lda, d, &
      ...
      if (s>0 .and. s<p) then
         ...
         do j = 1,i
            nullify(buf(this_thread)%chkptr)
            call ma64_swap(herm,n,nb,0,1,0,a,lda,aleft,aln, &
      ...
      ! Factor first half
      en2 = p/2
      call rfact_block(herm, pos_def, n, en2, nb, 0, a, lda, d, buf, perm, q1, &
      ...
      if (q1.ge.1) then
         if(pos_def) then
            call myzherk(mu, nu, q1, -rone, a(ip), lda, &
               rone, a(ip2), lda, control%min_ldsrk_work)
         else
            call ldsrk(herm, mu, nu, q1, -one, a(ip), lda, d, &
      ...
      ip = q1*lda + q1 + 1
      call rfact_block(herm, pos_def, n-q1, p-q1, nb, 0, a(ip), lda, &
      ...
      if (p-q1.gt.0 .and. .not.present(sublevel)) then
         ...
         call factor_solve_block(herm, n-q1, p-q1, nb, 0, a(ip), lda, &
...
!
subroutine factor_solve_block(herm, n, p, nb, s, a, lda, d, buf, perm,  &
   ...
   integer, intent(in) :: s ! number of cols in trapeziodal matrix that
      ...
      ! end and are search last for pivots.
   integer, intent(in) :: lda  ! leading dimension of a
   complex(wp), intent(inout) :: a(*) ! holds trapezoidal matrix 
      ! to be factorized. Holds the factorization on exit.
      ! Delayed columns are permuted to the final columns.
   complex(wp), intent(inout) :: d(2*p)
   complex (wp) :: buf(p*p) ! Work array
   integer, intent(inout) :: perm(*) ! perm array. Any swaps performed
      ! on columns of matrix are also performed on this array.
   integer, intent(out) :: q ! number of pivots chosen
   type(ma97_control), intent(in) :: control
   type(thread_stats), intent(inout) :: stats
   complex(wp) :: aleft(*) ! set of rows that need swapping too, leading dim lda
   ...
   complex(wp) aval ! Temporary work variable
   ...
   if (p.eq.1) then
      call factor_solve_block1(herm, n, a, d, q, control, stats)
      return
   end if

   u = min(max(control%u,rzero),0.5_wp)
   ...
   if (s>0 .and. s<p) then
      ...
      ! Make first i columns be last
      do j = 1,i
         call ma64_swap(herm,n,nb,q,1,0,a,lda,aleft,aln,buf,ldbuf,perm,j,p+1-j)
      end do
   end if
   ...
   pivot: do
      ...
      sweep: do mdummy = 1, p-q ! Look for a pivotal column or pair of columns
         ...
         k = kkr1+m-rm-1
         if (q>rm) then
            !call zgemv('NoTrans',n-m+1,q-rm,-one,a(k),lda, &
            !   buf(lda*(rm+0_long)+m),lda,one,a(kkm),1)
            call myzgemv(n-m+1,q-rm,a(k),lda, &
               buf(ldbuf*(rm+0_long)+m),ldbuf,a(kkm))
         end if
         if(herm) a(kkm) = real(a(kkm))
         ...
         if (t>0 .or. m.eq.p) then
            ...
            if (ubest1>u) then
               ...
               pivsiz = 1
               if (mbest.ne.m) &
                  call ma64_swap(herm,n,nb,q,1,rm,a,lda,aleft,aln,buf,ldbuf, &
                     perm,mbest,m)
               exit sweep
            end if
         end if
      ...
      pivsizes: if (pivsiz.eq.1) then
         ...
         if (q+1.ne.m) &
            call ma64_swap(herm,n,nb,q,1,rm,a,lda,aleft,aln,buf,ldbuf,perm, &
               q+1,m)
         ! We store D**-1.
         !print *, "1x1 pivot ", a(kkq1), "(col ", m, ")"
         d(2*q+1) = one/a(kkq1)
         d(2*q+2) = zero
         if (herm .and. real(a(kkq1)).lt.rzero) &
         ...
         ! Update columns q+2 to m (ie keep tp to date columns that have
         ! already been tested but failed)
         kkj = kkq1
         do j = q+2, min(nb,m)
            kkj = kkj + lda + 1
            call zaxpy(n-j+1,-buf(kb1+j-q-1),a(kkq1+j-q-1),1,a(kkj),1)
         end do
         ! Update q and kkq1
         kkq1 = kkq1 + lda + 1
         q = q+1
         ...
         if(herm) then
            do i = 2, p-q-1
               buf(kb1+i  ) = conjg(a(kkq1+i))
               buf(kb1+i+ldbuf) = conjg(a(k+i-1))
               a(kkq1+i) = d(2*q+1)*a(kkq1+i) + d(2*q+2)*a(k+i-1)
               a(k+i-1) = d(2*q+3)*a(k+i-1) + conjg(d(2*q+2)*buf(kb1+i))
            end do
            do i = p-q, n-q-1
               aval = conjg(a(kkq1+i))
               a(kkq1+i) = d(2*q+1)*a(kkq1+i) + d(2*q+2)*a(k+i-1)
               a(k+i-1) = d(2*q+3)*a(k+i-1) + conjg(d(2*q+2)*aval)
            end do
         else
            do i = 2, p-q-1
               buf(kb1+i  ) = a(kkq1+i)
         ...
         j = max(q+3,1)
         if (m>=j) then
            call zgemm('n','t',n-j+1,m-j+1,2,-one,a(kkq1+j-q-1), &
...
! based on hsl_ma64 version 6.0.0 (ported 18th January 2011)
!
subroutine factor_solve_block1(herm, n, a, d, q, control, stats)
   logical, intent(in) :: herm
   integer, intent(in) :: n ! number of rows in trapeziodal matrix
   complex(wp), intent(inout) :: a(*) ! holds trapezoidal matrix 
     ! to be factorized. Holds the factorization on exit.
   ...
   if (abs(a(1)).le.control%small) then
      ...
      do k = 2, n
         if (abs(a(k)).gt.control%small) then
            if (abs(a(k)) > huge(rzero)) stats%flag = MA97_ERROR_INFINITY
            return ! Delay column
         end if
      end do
   ...
   ! Good 1x1 pivot
   if (herm .and. abs(a(1)).lt.0) stats%num_neg = stats%num_neg + 1
   ! Store L in A
   d(1) = 1/a(1)
   d(2) = zero
   a(1) = 1/a(1)
...
!
subroutine ma64_swap(herm,n,nb,q,q1,rm,a,lda,aleft,aln,buf,ldbuf,perm,j1,j2)
   logical, intent(in) :: herm
   integer, intent(in) :: n
   ...
   call zswap(q-q1+1-rm,buf(j1+rm*ldbuf),ldbuf,buf(j2+rm*ldbuf),ldbuf)
   !call zswap(q-q1+1,buf(j1),n,buf(j2),n)
   ...
   l = j1-1
   if (l>0) call zswap(l,a(k1),lda,a(k2),lda)
   if (aln>0) call zswap(aln,aleft(j1),lda,aleft(j2),lda)
   ...
   jb = j2-j1-1
   l = min(jb,nb-j1,j2-1)
   if (l>0.and.j2.ge.1) call zswap(l,a(k1),1,a(k2),lda)
   if(herm) then
      a(k1:k1+l-1) = conjg(a(k1:k1+l-1))
      a(k2:k2+(l-1)*lda:lda) = conjg(a(k2:k2+(l-1)*lda:lda))
   ...
   if (n>j2) call zswap(n-j2,a(k1),1,a(k2),1)
...
subroutine ma64_update(n,nb,kkq,a,lda,buf,ldbuf,jl,jr,pl,pr)
   ...
   do i = 1, j2-j+1, bs
      ...
      p = min(bs, j2-j+1-i+1)
      call zgemm ('n', 't', n-j+1-i+1, p, pr-pl+1, -one, &
...
!*******************************
! Equivalent to call:
! call zgemv('NoTrans', m, n, -one, a, lda, b, ldb, one, c, 1)
! However for small updates exploit explicit loops as this is faster
! (calls zgemv itself for large updates)
subroutine myzgemv(m, n, a, lda, b, ldb, c)
   integer, intent(in) :: m
   integer, intent(in) :: n
   ...
   integer :: i
   integer :: ipa, ipb
   complex(wp) :: b1, b2, b3, b4, val

   select case(n)
   case(1)
      !call zgemv('NoTrans', m, 1, -one, a, lda, b, ldb, one, c, 1)
      b1 = b(1)
      c(1:m) = c(1:m) - b1 * a(1:m)
   case(2)
      !call zgemv('NoTrans', m, 2, -one, a, lda, b, ldb, one, c, 1)
      b1 = b(1)
      b2 = b(1+ldb)
...
subroutine stack_alloc(stack, ptr, len, st)
   ...
   complex(wp), dimension(:), pointer, intent(out) :: ptr
...
!*************************************************
!
! This routine frees all memory associated with the stack.
! It is designed to be called if a factorization needs to abort for some reason
! (e.g. finds matrix is pos def or singular)
!
subroutine stack_free_all(stack)
   type(stack_mem_type), pointer, intent(inout) :: stack

   type(stack_mem_type), pointer :: next

   next => stack
   do while(associated(next))
      stack => next
      next => stack%below
      deallocate(stack) ! Automatically deallocates components
   end do
   nullify(stack)
end subroutine stack_free_all
...
pure integer function ma97_get_n_double_complex(akeep)
   type(ma97_akeep), intent(in) :: akeep

   ma97_get_n_double_complex = akeep%n
end function ma97_get_n_double_complex

pure integer function ma97_get_nz_double_complex(akeep)
   type(ma97_akeep), intent(in) :: akeep

   ma97_get_nz_double_complex = akeep%ne
end function ma97_get_nz_double_complex

end module hsl_ma97_double_complex
