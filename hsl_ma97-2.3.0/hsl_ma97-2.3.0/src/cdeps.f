...
C**********************************************************************
      SUBROUTINE MF64A(JOB,N,NE,IP,IRN,A,NUM,CPERM,LIW,IW,LDW,DW,
     &           ICNTL,INFO)
      IMPLICIT NONE
C
...
C   subroutine.
C
C A is a COMPLEX array of length NE.
...
C External routines and functions
      EXTERNAL MC21A,MF64B,MC64R,MC64S,MC64W
...
C IW(1:5N), DW(1:N) are workspaces
        CALL MF64B(N,NE,IP,IRN,A,CPERM,NUM,
...
C**********************************************************************
      SUBROUTINE MF64B(N,NE,IP,IRN,A,IPERM,NUM,JPERM,PR,Q,L,D)
      IMPLICIT NONE
C
C *** Copyright (c) 1999  Council for the Central Laboratory of the
C     Research Councils                                             ***
...
C A is a COMPLEX array of length
