...
!    change real to integer, kind(0)
   ...
   module hsl_mc34_double_complex
   implicit none
   private
   public mc34_expand
   ...
   interface mc34_expand
      module procedure mc34_expand_double_complex
   end interface

   contains

      subroutine mc34_expand_double_complex(n,row,ptr,iw,a,sym_type)
...
! note that i is always greater than or equal to j
      ...
      end subroutine mc34_expand_double_complex
   end module hsl_mc34_double_complex
...
!
module hsl_mc69_double_complex
   implicit none
...
!
subroutine mc69_verify_double_complex(lp, matrix_type, m, n, ptr, row, flag, &
      more, val)
   ...
   complex(wp), dimension(:), optional, intent(in) :: val ! matrix values,if any
   ...
   if(present(val)) then
      ...
      case(HSL_MATRIX_CPLX_HERM_PSDEF)
         ! Check for positive diagonal entries and zero imaginary components
         do j = 1,n
            k = ptr(j)
            ! Note: column cannot be empty as previously checked pattern ok
            if(aimag(val(k)).ne.zero) then
               flag = MC69_ERROR_IMAG_DIAGONAL
               more = j
               call mc69_print_flag(context,lp,flag)
               return
            endif
      ...
      case(HSL_MATRIX_CPLX_HERM_INDEF)
         ! Check for zero imaginary components
         do j = 1,n
            k = ptr(j)
            ! Note: we're OK if the column is empty - row(k) is still an
            ! entry, however we must be careful that we've not gone past the
            ! end of the matrix
            if(k.ge.ptr(n+1)) exit
            if(row(k)/=j) cycle ! No diagonal entry
            if(aimag(val(k)).ne.zero) then
               flag = MC69_ERROR_IMAG_DIAGONAL
               more = k
               call mc69_print_flag(context,lp,flag)
...
end subroutine mc69_verify_double_complex
...
!
subroutine mc69_print_double_complex(lp, lines, matrix_type, m, n, ptr, row, &
      val, cbase)
   ...
   complex(wp), dimension(ptr(n+1)-1), optional, intent(in) :: val ! matrix vals
   ...
   character(len=30) :: negfrmt, valfrmt, emptyfrmt
   integer ::  rebase
   ...
   case(HSL_MATRIX_CPLX_RECT)
      write(lp, "(a)", advance="no") &
         "Complex rectangular matrix, dimension "
   case(HSL_MATRIX_CPLX_UNSYM)
      write(lp, "(a)", advance="no") &
         "Complex unsymmetric matrix, dimension "
   case(HSL_MATRIX_CPLX_HERM_PSDEF)
      write(lp, "(a)", advance="no") &
         "Complex Hermitian positive-definite matrix, dimension "
   case(HSL_MATRIX_CPLX_HERM_INDEF)
      write(lp, "(a)", advance="no") &
         "Complex Hermitian indefinite matrix, dimension "
   case(HSL_MATRIX_CPLX_SYM)
      write(lp, "(a)", advance="no") &
         "Complex symmetric matrix, dimension "
   case(HSL_MATRIX_CPLX_SKEW)
      write(lp, "(a)", advance="no") &
         "Complex skew symmetric matrix, dimension "
   case default
      write(lp, "(a,i5)") &
   ...
   if(((present(val) .and. n.lt.5) .or. (.not.present(val) .and. n.lt.24)) &
         ...
         if(present(val)) then
            do j = 1, n
               ...
               elseif(dmat(k,j).gt.0) then
                  if(real(val(dmat(k,j))).gt.0.0_wp .and. &
                        aimag(val(dmat(k,j))).gt.0.0_wp) then
                  ...
                  ! in upper triangle
                  select case(matrix_type)
                  case(HSL_MATRIX_CPLX_HERM_PSDEF,HSL_MATRIX_CPLX_HERM_INDEF)
                     if(real(val(-dmat(k,j))).gt.0.0_wp .and. &
                           -aimag(val(-dmat(k,j))).gt.0.0_wp) then
                        write(lp,valfrmt,advance="no") conjg(val(-dmat(k,j)))
                     else
                        write(lp,negfrmt,advance="no") conjg(val(-dmat(k,j)))
                     endif
                  case(HSL_MATRIX_CPLX_SYM)
                     if(real(val(-dmat(k,j))).gt.0.0_wp .and. &
                           aimag(val(-dmat(k,j))).gt.0.0_wp) then
      ...
      write(lp, "(a)") "First 3 entries in columns:"
      llines = llines - 1
      do col = 1, llines
         write(lp, "(a)", advance="no") "Col "
         write(lp, nfrmt, advance="no") col-rebase
         write(lp, "(':')", advance="no")
         do j = ptr(col), min(ptr(col+1)-1, ptr(col)+2)
            write(lp, "(' ')", advance="no")
            write(lp, mfrmt, advance="no") row(j)-rebase
            if(present(val)) then
               write(lp, "(' (',es9.2,',',es9.2,')')", advance="no") val(j)
            endif
         end do
         write(lp, "()")
      end do
   endif
end subroutine mc69_print_double_complex
...
subroutine mc69_cscl_clean_double_complex(matrix_type, m, n, ptr, row, flag, &
   ...
   integer, intent(out) :: flag ! return code
   complex(wp), optional, dimension(*), intent(inout) :: val ! values
   ...
   if(matrix_type > 0 .or. matrix_type < -6) then
      flag = MC69_ERROR_MATRIX_TYPE
...
end subroutine mc69_cscl_clean_double_complex
...
!
subroutine mc69_cscl_convert_double_complex(matrix_type, m, n, ptr_in, row_in, &
   ...
   integer, intent(out) :: flag ! return code
   complex(wp), optional, dimension(*), intent(in) :: val_in ! values input
   complex(wp), optional, allocatable, dimension(:) :: val_out
      ! values on output
   ...
   call mc69_cscl_convert_main(context, 1, matrix_type, m, n, &
      ...
      lp, noor, ndup) 
end subroutine mc69_cscl_convert_double_complex
      ...
      do col = 1, n
         ...
         case(HSL_MATRIX_CPLX_HERM_PSDEF, HSL_MATRIX_CPLX_HERM_INDEF)
            do i = ptr_in(col), ptr_in(col+1)-1
               ...
               row_out(k) = row_in(i)
               val_out(k) = &
                  cmplx(real(val_in(i)), multiplier*aimag(val_in(i)), kind=wp)
               k = k + 1
            end do
         case default
            do i = ptr_in(col), ptr_in(col+1)-1
   ...
   if(present(val_out)) then
      ...
      case(HSL_MATRIX_CPLX_HERM_PSDEF)
         ...
         do j = 1,n
            ...
            if(aimag(val_out(k)).ne.0.0) then
               flag = MC69_ERROR_IMAG_DIAGONAL
               call mc69_print_flag(context,nout,flag)
               return
            endif
      ...
      case(HSL_MATRIX_CPLX_HERM_INDEF)
         ...
         do j = 1,n
            ...
            ! entry, however we must be careful that we've not gone past the
            ! end of the matrix
            if(k.ge.ptr_out(n+1)) exit
            if(row_out(k)/=j) cycle ! No diagonal entry
            if(aimag(val_out(k)).ne.zero) then
               flag = MC69_ERROR_IMAG_DIAGONAL
               call mc69_print_flag(context,nout,flag)
               return
            endif
...
!
subroutine mc69_cscu_convert_double_complex(matrix_type, n, ptr_in, row_in, &
      ptr_out, row_out, flag, val_in, val_out, lmap, map, lp, noor, ndup) 
   integer, intent(in) :: matrix_type ! what sort of symmetry is there?
   ...
   ! Note: have to change this test for complex code
   if(matrix_type > -3 .or. matrix_type < -6) then
      flag = MC69_ERROR_MATRIX_TYPE
      call mc69_print_flag(context,nout,flag)
      return
   end if
   ...
   call mc69_csrl_convert_main(context, -1, matrix_type, n, n, &
      ptr_in, row_in, ptr_out, row_out, flag, val_in, val_out, lmap, map, &
      lp, noor, ndup) 
end subroutine mc69_cscu_convert_double_complex
...
! within each column ordered by increasing row index (HSL standard format)
!
subroutine mc69_csclu_convert_double_complex(matrix_type, n, ptr_in, row_in, &
      ptr_out, row_out, flag, val_in, val_out, lmap, map, lp, noor, ndup) 
   integer, intent(in) :: matrix_type ! what sort of symmetry is there?
   integer, intent(in) :: n ! matrix dimension
   integer, dimension(*), intent(in) :: ptr_in ! column pointers on input
   ...
   call mc69_csclu_convert_main(context, -1, matrix_type, n, &
      ptr_in, row_in, ptr_out, row_out, flag, val_in, val_out, lmap, map, &
      lp, noor, ndup) 
end subroutine mc69_csclu_convert_double_complex
...
! within each column ordered by increasing row index (HSL standard format)
!
subroutine mc69_csrl_convert_double_complex(matrix_type, m, n, ptr_in, col_in, &
      ptr_out, row_out, flag, val_in, val_out, lmap, map, lp, noor, ndup)
   integer, intent(in) :: matrix_type ! what sort of symmetry is there?
   ...
   call mc69_csrl_convert_main(context, 1, matrix_type, m, n, &
      ...
      lp, noor, ndup) 
end subroutine mc69_csrl_convert_double_complex
...
! Hermitian matrices to HSL standard format
!
subroutine mc69_csru_convert_double_complex(matrix_type, n, ptr_in, col_in, &
      ptr_out, row_out, flag, val_in, val_out, lmap, map, lp, noor, ndup) 
   integer, intent(in) :: matrix_type ! what sort of symmetry is there?
   integer, intent(in) :: n ! number of columns
   integer, dimension(*), intent(in) :: ptr_in ! column pointers on input
   ...
   call mc69_cscl_convert_main(context, -1, matrix_type, n, n, &
      ptr_in, col_in, ptr_out, row_out, flag, val_in, val_out, lmap, map, &
      lp, noor, ndup) 
end subroutine mc69_csru_convert_double_complex
...
! within each column ordered by increasing row index (HSL standard format)
!
subroutine mc69_csrlu_convert_double_complex(matrix_type, n, ptr_in, col_in, &
      ptr_out, row_out, flag, val_in, val_out, lmap, map, lp, noor, ndup) 
   integer, intent(in) :: matrix_type ! what sort of symmetry is there?
   integer, intent(in) :: n ! matrix dimension
   integer, dimension(*), intent(in) :: ptr_in ! row pointers on input
   ...
   call mc69_csclu_convert_main(context, 1, matrix_type, n, &
      ptr_in, col_in, ptr_out, row_out, flag, val_in, val_out, lmap, map, &
      lp, noor, ndup) 
end subroutine mc69_csrlu_convert_double_complex
...
!
subroutine mc69_coord_convert_double_complex(matrix_type, m, n, ne, row, col, &
      ptr_out, row_out, flag, val_in, val_out, lmap, map, lp, noor, ndup) 
   integer, intent(in) :: matrix_type ! what sort of symmetry is there?
   integer, intent(in) :: m ! number of rows in matrix
...
end subroutine mc69_coord_convert_double_complex
...
!
subroutine mc69_set_values_double_complex(matrix_type, lmap, map, val, ne, &
      val_out)
   ...
   complex(wp), dimension(ne), intent(out) :: val_out
   ...
   case(HSL_MATRIX_CPLX_HERM_PSDEF, HSL_MATRIX_CPLX_HERM_INDEF)
      ...
      do i = 1, ne
         j = abs(map(i))
         val_out(i) = cmplx( real(val(j)), &
            sign(1.0,real(map(i))) * aimag(val(j)), kind=wp )
      end do
      ...
      do i = ne+1, lmap, 2
         j = abs(map(i))
         k = abs(map(i+1))
         val_out(j) = val_out(j) + cmplx( real(val(k)), &
            sign(1.0,real(map(i+1))) * aimag(val(k)), kind=wp )
      end do
   case(HSL_MATRIX_CPLX_SKEW)
      !
...
end subroutine mc69_set_values_double_complex
...
subroutine sort( array, n, map, val )
   ...
   complex(wp), dimension(n), optional, intent(inout) :: val ! Apply same
      ! permutation to val
...
subroutine pushdown(root, last, array, val, map)
   ...
   complex(wp), dimension(last), optional, intent(inout) :: val
   ...
   complex(wp) :: root_val ! value of val(root) at start of iteration
...
end module hsl_mc69_double_complex
...
MODULE HSL_ZD11_double_complex
...
!  ==========================
  ...
  TYPE, PUBLIC :: ZD11_type
    ...
    COMPLEX ( KIND( 1.0D+0 ) ), ALLOCATABLE, DIMENSION(:) :: val
  END TYPE
...
END MODULE HSL_ZD11_double_complex
...
! * s/_double/_double/g
! * Change wp
! * Change: MF64AD, MC64DD, MC64ED, MC64FD, MC64RD, MC64QD, mc64ad, mf64ad,
!           mc64wd
!

module hsl_mc64_double_complex
   use hsl_mc34_double
...
!     real(wp) :: relax = 0d0 ! Relaxes matching
   ...
   interface mc64_matching
      module procedure mc64_matching_zd11_double_complex
      module procedure mc64_matching_hslstd_double_complex
...
!       control%relax = default%relax
   ...
   subroutine mc64_matching_zd11_double_complex(job,matrix,control,info,perm, &
         scale)
      ...
      call mc64_matching_hslstd_double_complex(job,matrix_type,matrix%m, &
         matrix%n,matrix%ptr,matrix%row,matrix%val,control,info,perm,scale)
   end subroutine mc64_matching_zd11_double_complex

   subroutine mc64_matching_hslstd_double_complex(job,matrix_type,m,n,ptr,row, &
...
! Expand matrix if symmetric
      if (sym) then
        ...
        dw(1:ptr(n+1)-1) = abs(val(1:ptr(n+1)-1))
        iw(1:n+1) = ptr(1:n+1)
...
! Set column permutation and row permutation to be the same
          ...
          call mf64ad(job,n,ne,ptr,row,val,num,  &
...
! Set column permutation to identity
  ...
  100 info%flag = -5
   ...
   end subroutine mc64_matching_hslstd_double_complex
...
!     MC21A/MC64ZD is used for this. See [1].
...
! Compute bottleneck matching
      IF (JOB.EQ.2) THEN
! Pass CNTL(1) to MC64BD through DW(1)
...
! Pass CNTL(1) to MC64SD through FACT
...
! Pass CNTL(1) to MC64WD through DW(1)
! Pass JOB to MC64WD through IW(1)
         DW (1) = MAX (ZERO, CNTL (1) )
         IW (1) = JOB
! Call MC64WD
...
! IW4 is integer work array of length 3N+M used by MC64UD.
!
...
! CNT is the number of calls made to MC64UD so far.
...
! If LENH(J) = LENL(J), remove J from W
   ...
   80       END DO
! End of dummy loop; this point is never reached
! Set mode for next call to MC64UD
            MOD = 1
         ELSE
...
! Initialize CV and ARP; parameters MOD, NUMX are not accessed
         ...
         NUM2 = N
      ELSE
! Not the first call to MC64UD.
! Re-initialize ARP if entries were deleted since last call to MC64UD
         IF (MOD.EQ.1) THEN
            DO J = 1, N
               ARP (J) = 0
            END DO
...
! NFC is number of rows/columns that could not be assigned
      NFC = 0
! Integers ID0+1 to ID0+N are unique numbers for call ID to MC64UD,
...
end module hsl_mc64_double_complex
...
! To convert from double to single:
! * Change wp
! * Replace _double_complex by _complex
! * Replace _double by _single
! * Replace call to mc64wd to mc64w

module hsl_mc80_double_complex
   use hsl_mc34_double
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   interface mc80_order
      ...
      module procedure mc80_order_double_complex
   end interface

   interface mc80_order_full
      ! to be used if lower and upper triangles of A available
      module procedure mc80_order_full_double_complex
   end interface
...
!
subroutine mc80_order_double_complex(ord, n, ptr, row, val, order, &
      control, info, scale)
...
end subroutine mc80_order_double_complex
...
!  
subroutine mc80_order_full_double_complex(ord, n, ptr, row, val, order,  &
      control, info, scale)
   integer, intent(in) :: ord ! controls ordering on compressed matrix
         ! 1 An approximate minimum degree ordering is used.
         ! 2 A minimum degree ordering is used (as in MA27).
...
end subroutine mc80_order_full_double_complex
...
!**********************************************************************
end module hsl_mc80_double_complex
