...
!
subroutine mc69_verify_complex(lp, matrix_type, m, n, ptr, row, flag, more, val)
...
end subroutine mc69_verify_complex
...
!
subroutine mc69_print_complex(lp, lines, matrix_type, m, n, ptr, row, val, &
      cbase)
...
end subroutine mc69_print_complex
...
subroutine mc69_cscl_clean_complex(matrix_type, m, n, ptr, row, flag, &
...
end subroutine mc69_cscl_clean_complex
...
!
subroutine mc69_cscl_convert_complex(matrix_type, m, n, ptr_in, row_in, &
   ...
   call mc69_cscl_convert_main(context, 1, matrix_type, m, n, &
      ...
      lp, noor, ndup) 
end subroutine mc69_cscl_convert_complex
...
!
subroutine mc69_cscu_convert_complex(matrix_type, n, ptr_in, row_in, ptr_out, &
   ...
   call mc69_csrl_convert_main(context, -1, matrix_type, n, n, &
      ptr_in, row_in, ptr_out, row_out, flag, val_in, val_out, lmap, map, &
      lp, noor, ndup) 
end subroutine mc69_cscu_convert_complex
...
! within each column ordered by increasing row index (HSL standard format)
!
subroutine mc69_csclu_convert_complex(matrix_type, n, ptr_in, row_in, ptr_out, &
      row_out, flag, val_in, val_out, lmap, map, lp, noor, ndup) 
   integer, intent(in) :: matrix_type ! what sort of symmetry is there?
   integer, intent(in) :: n ! matrix dimension
   integer, dimension(*), intent(in) :: ptr_in ! column pointers on input
   ...
   call mc69_csclu_convert_main(context, -1, matrix_type, n, &
      ptr_in, row_in, ptr_out, row_out, flag, val_in, val_out, lmap, map, &
      lp, noor, ndup) 
end subroutine mc69_csclu_convert_complex
...
! within each column ordered by increasing row index (HSL standard format)
!
subroutine mc69_csrl_convert_complex(matrix_type, m, n, ptr_in, col_in, &
      ptr_out, row_out, flag, val_in, val_out, lmap, map, lp, noor, ndup)
   integer, intent(in) :: matrix_type ! what sort of symmetry is there?
   ...
   call mc69_csrl_convert_main(context, 1, matrix_type, m, n, &
      ...
      lp, noor, ndup) 
end subroutine mc69_csrl_convert_complex
...
! Hermitian matrices to HSL standard format
!
subroutine mc69_csru_convert_complex(matrix_type, n, ptr_in, col_in, ptr_out, &
      row_out, flag, val_in, val_out, lmap, map, lp, noor, ndup) 
   integer, intent(in) :: matrix_type ! what sort of symmetry is there?
   integer, intent(in) :: n ! number of columns
   integer, dimension(*), intent(in) :: ptr_in ! column pointers on input
   ...
   call mc69_cscl_convert_main(context, -1, matrix_type, n, n, &
      ptr_in, col_in, ptr_out, row_out, flag, val_in, val_out, lmap, map, &
      lp, noor, ndup) 
end subroutine mc69_csru_convert_complex
...
! within each column ordered by increasing row index (HSL standard format)
!
subroutine mc69_csrlu_convert_complex(matrix_type, n, ptr_in, col_in, ptr_out, &
      row_out, flag, val_in, val_out, lmap, map, lp, noor, ndup) 
   integer, intent(in) :: matrix_type ! what sort of symmetry is there?
   integer, intent(in) :: n ! matrix dimension
   integer, dimension(*), intent(in) :: ptr_in ! row pointers on input
   ...
   call mc69_csclu_convert_main(context, 1, matrix_type, n, &
      ptr_in, col_in, ptr_out, row_out, flag, val_in, val_out, lmap, map, &
      lp, noor, ndup) 
end subroutine mc69_csrlu_convert_complex
...
!
subroutine mc69_coord_convert_complex(matrix_type, m, n, ne, row, col, ptr_out,&
      row_out, flag, val_in, val_out, lmap, map, lp, noor, ndup) 
   integer, intent(in) :: matrix_type ! what sort of symmetry is there?
   integer, intent(in) :: m ! number of rows in matrix
...
end subroutine mc69_coord_convert_complex
...
!
subroutine mc69_set_values_complex(matrix_type, lmap, map, val, ne, val_out)
...
end subroutine mc69_set_values_complex
...
!  ==========================
  ...
  TYPE, PUBLIC :: ZD11_type
    ...
    COMPLEX ( KIND( 1.0E+0 ) ), ALLOCATABLE, DIMENSION(:) :: val
  END TYPE
...
! * s/_double/_single/g
! * Change wp
! * Change: MF64A, MC64D, MC64E, MC64F, MC64R, MC64Q, mc64a, mf64a, mc64w
!

module hsl_mc64_complex
   use hsl_mc34_single
...
!     real(wp) :: relax = 0d0 ! Relaxes matching
   ...
   interface mc64_matching
      module procedure mc64_matching_zd11_complex
      module procedure mc64_matching_hslstd_complex
...
!       control%relax = default%relax
   ...
   subroutine mc64_matching_zd11_complex(job,matrix,control,info,perm, &
         scale)
      ...
      call mc64_matching_hslstd_complex(job,matrix_type,matrix%m, &
         matrix%n,matrix%ptr,matrix%row,matrix%val,control,info,perm,scale)
   end subroutine mc64_matching_zd11_complex

   subroutine mc64_matching_hslstd_complex(job,matrix_type,m,n,ptr,row, &
...
! Set column permutation and row permutation to be the same
          ...
          call mf64a(job,n,ne,ptr,row,val,num,  &
...
! Set column permutation to identity
  ...
  100 info%flag = -5
   ...
   end subroutine mc64_matching_hslstd_complex
...
!
subroutine mc80_order_complex(ord, n, ptr, row, val, order, &
      control, info, scale)
...
end subroutine mc80_order_complex
...
!  
subroutine mc80_order_full_complex(ord, n, ptr, row, val, order,  &
      control, info, scale)
   integer, intent(in) :: ord ! controls ordering on compressed matrix
         ! 1 An approximate minimum degree ordering is used.
         ! 2 A minimum degree ordering is used (as in MA27).
...
end subroutine mc80_order_full_complex
