...
! * Change wp
! * Change _single
! * Change BLAS calls: saxpy, sgemm, sgemv, spotrf, sswap, ssyrk, strsm, strsv,
!                      snrm2, strmv, strmm
...
!
subroutine analyse_single(check, n, ptr, row, akeep, control, info, &
      order, val)
...
! required by the factorization are set up.
!
subroutine ma97_analyse_coord_single(n, ne, row, col, akeep, control, &
      info, order, val)
   integer, intent(in) :: n ! order of A
...
!
subroutine ma97_factor_single(matrix_type, val, akeep, fkeep, control, &
      info, scale, ptr, row)
   ...
   call ma97_factor_solve_single(matrix_type, val, 0, x, 0, akeep, &
...
!
subroutine ma97_factor_solve_one_single(matrix_type,val,x1,akeep, &
   ...
   call ma97_factor_solve_single(matrix_type,val,1,x1,ldx1,akeep, &
      fkeep,control,info,scale=scale,ptr=ptr,row=row)
...
!
subroutine ma97_factor_solve_single(matrix_type, val, nrhs, x, ldx, &
...
subroutine mc30_scale(n, invp, ptr, row, val, scaling, info, st)
   ...
   call mc30a(n, nnz, val, row, col, scale30, work, 6, info)
...
!%%!$OMP    PRIVATE(ltask)                                                    &
!$OMP    PRIVATE(this_thread)                                                 &
!$OMP    SHARED(abort, alloc2, buf, child_list, child_ptr, control, invp,     &
...
subroutine slv_fwd_mf(pos_def, nelim, l, ldl, nrhs, x, ldx, xstack, ldxs, blas3)
   ...
   if(nrhs.eq.1 .and. .not.blas3) then
      ! Perform solve
      if(pos_def) then
         call strsv('Lower', 'Non-Trans', 'Non-Unit', nelim, l, ldl, x, 1)
      else
         call strsv('Lower', 'Non-Trans', 'Unit', nelim, l, ldl, x, 1)
      end if

      ! Update x values corresponding to delays
      if(ldx.gt.nelim) then
         call sgemv('N', ldx-nelim, nelim, -one, l(nelim+1), ldl, &
            x, 1, one, x(nelim+1), 1)
      end if
      ...
      if(ldxs.gt.0) then
         if(nelim.ne.0) then
            call sgemv('N', ldxs, nelim, -one, l(ldx+1), ldl, x, 1, &
               zero, xstack, 1)
         else
            xstack(1:ldxs) = zero
         end if
      ...
      ! Perform solve
      if(pos_def) then
         call strsm('Left', 'Lower', 'Non-Trans', 'Non-Unit', nelim, nrhs, &
            one, l, ldl, x, ldx)
      else
         call strsm('Left', 'Lower', 'Non-Trans', 'Unit', nelim, nrhs, &
            one, l, ldl, x, ldx)
      end if

      ! Update x values corresponding to delays
      if(ldx.gt.nelim) then
         call sgemm('N', 'N', ldx-nelim, nrhs, nelim, -one, l(nelim+1), ldl, &
            x, ldx, one, x(nelim+1), ldx)
      end if

      ! Update uneliminated contributions
      if(ldxs.gt.0) then
         call sgemm('N', 'N', ldxs, nrhs, nelim, -one, l(ldx+1), ldl, x, ldx, &
            zero, xstack, ldxs)
      end if
   end if
...
! In the serial case could just use ssyrk (but don't for bit-compatibility).
! Split as:
! ( L_1 ) ( L_1' L_2' ) + ( C_11      )
! ( L_2 )                 ( C_21 C_22 )
...
! Note: n <= m
!
subroutine myssyrk(m, n, k, alpha, l, ldl, beta, c, ldc, &
      min_ldsrk_work)
...
!$OMP    SHARED(alpha, beta, c, k, l, ldc, ldl)
      ...
      ip = (i-1)*ldc + i
      call ssyrk('L', 'N', blkn, k, alpha, l(i), ldl, beta, c(ip), ldc)
      !%%if(dolog) call log_stop(ltask, LOG_UNIT)
!$OMP END TASK
      do j = i+blkn, m, minsz
...
!$OMP       SHARED(alpha, beta, c, k, l, ldc, ldl)
         !%%if(dolog) call log_start(ltask, "DL")
         ip = (i-1)*ldc + j
         call sgemm('N', 'T', blkm, blkn, k, alpha, l(j), ldl, &
...
!
subroutine ma97_solve_one_single(x, akeep, fkeep, control, info, job)
   ...
   ldx = size(x)
   call ma97_solve_mult_single(1, x, ldx, akeep, fkeep, control, info, &
      job=job)
end subroutine ma97_solve_one_single

!*************************************************************************

subroutine ma97_solve_mult_single(nrhs, x, ldx, akeep, fkeep, control, &
      info, job)
   integer, intent(in) :: nrhs
   integer, intent(in) :: ldx
...
subroutine ma97_solve_fredholm_single(nrhs, flag_out, x, ldx, &
   ...
   if (fkeep%flag.ne.MA97_WARNING_FACT_SINGULAR) then
      call ma97_solve_mult_single(nrhs, x, ldx, akeep, fkeep, control, info)
      return
   end if
   ...
   if (allocated(fkeep%scaling)) then
    ...
    do i = 1,nrhs
       tol(i) = control%consist_tol * n * snrm2(n,x(:,i),1)
    end do
...
subroutine ma97_lmultiply_one_single(trans, x1, y1, akeep, fkeep, &
   ...
   ldx = size(x1)
   ldy = size(y1)
   call ma97_lmultiply_mult_single(trans,1,x1,ldx,y1,ldy, &
      akeep,fkeep,control,info)
...
recursive subroutine bwd_slv_tasks(chunk, chunk_sa, chunk_en, fwd_ptr, fwd, &
   ...
   if(nrhs.eq.1) then
      call subtree_bwd_slv_one(chunk_en(chunk), chunk_sa(chunk), job, pos_def, &
         nnodes, nodes, sptr, rptr, rlist, invp, x, ldx, blas3, st)
   else
      call subtree_bwd_slv_mult(chunk_en(chunk), chunk_sa(chunk), job, pos_def,&
         nnodes, nodes, sptr, rptr, rlist, invp, nrhs, x, ldx, st)
   endif
   if(st.ne.0) return
...
!
subroutine subtree_bwd_slv_one(en, sa, job, pos_def, nnodes, nodes, sptr, &
...
subroutine subtree_bwd_slv_mult(en, sa, job, pos_def, nnodes, nodes, sptr, &
...
subroutine node_mult(pos_def, trans, nvec, x, ldx, y, ldy, rlist, invp, &
   ...
   if(nvec.eq.1) then ! matrix-vector version
      if(.not.trans) then
         if (blkm-nelim.gt.0) &
            call sgemv('N', blkm-nelim, nelim, one, lcol(nelim+1), blkm, &
               xlocal, 1, zero, xlocal(nelim+1), 1)
         ! strmv comes last as it is destructive of xlocal
         call strmv('L', 'N', diag, nelim, lcol, blkm, xlocal, 1)
      else
         ! strmv comes first as sgemv is destructive of xlocal
         call strmv('L', 'T', diag, nelim, lcol, blkm, xlocal, 1)
         if (blkm-nelim.gt.0) &
            call sgemv('T', blkm-nelim, nelim, one, lcol(nelim+1), blkm, &
               xlocal(nelim+1), 1, one, xlocal, 1)
      end if
   else ! matrix-matrix version
      if(.not.trans) then
         if (blkm-nelim.gt.0) &
            call sgemm('N', 'N', blkm-nelim, nvec, nelim, one, lcol(nelim+1), &
               blkm, xlocal, blkm, zero, xlocal(nelim+1), blkm)
         ! strmv comes last as it is destructive of xlocal
         call strmm('Left', 'Lower', 'Non-T', diag, nelim, nvec, one, &
            lcol, blkm, xlocal, blkm)
      else
         ! strmv comes first as sgemv is destructive of xlocal
         call strmm('Left', 'Lower', 'Trans', diag, nelim, nvec, one, &
            lcol, blkm, xlocal, blkm)
         if (blkm-nelim.gt.0) &
            call sgemm('T', 'N', nelim, nvec, blkm-nelim, one, lcol(nelim+1), &
...
subroutine slv_fwd_one(pos_def, rlist, invp, x, blkm, blkn, nelim, nd, lcol, &
   ...
   if (blkm.gt.10 .and. nelim.gt.4) then
      ...
      if(blas3) then
         if(pos_def) then
            call strsm('Left', 'Lower', 'Non-Trans', 'Non-Unit', nelim, 1, &
               one, lcol, blkm, xlocal, blkm)
         else
            call strsm('Left', 'Lower', 'Non-Trans', 'Unit', nelim, 1, &
               one, lcol, blkm, xlocal, blkm)
         end if
      else
         if(pos_def) then
            call strsv('L','N','N', nelim, lcol, blkm, xlocal, 1)
         else
            call strsv('L','N','U', nelim, lcol, blkm, xlocal, 1)
         end if
      end if

      if (blkm-nelim.gt.0) then
         if(blas3) then
            call sgemm('N', 'N', blkm-nelim, 1, nelim, -one, &
...
subroutine slv_fwd_mult(pos_def, rlist, invp, nrhs, x, ldx, blkm, blkn, nelim, &
   ...
   if (blkm.gt.10 .and. nelim.gt.4) then
      ...
      if (blkm-nelim.gt.0) then
         call sgemm('N', 'N', blkm-nelim, nrhs, nelim, -one, &
            lcol(nelim+1), blkm, xlocal, blkm, zero, &
...
subroutine slv_bwd_one(pos_def, job, rlist, invp, x, blkm, blkn, nelim, &
   ...
   if (blkm.gt.10 .and. nelim.gt.4) then
      ...
      if (blkm-nelim.gt.0) then
         if(blas3) then
            call sgemm('Trans', 'Non-trans', nelim, 1, blkm-nelim, -one, &
      ...
      if(blas3) then
         if(pos_def) then
            call strsm('Left','Lower','Trans','Non-Unit', nelim, 1, one, &
               lcol, blkm, xlocal, blkm)
         else
            call strsm('Left','Lower','Trans','Unit', nelim, 1, one, lcol, &
               blkm, xlocal, blkm)
         end if
      else
         if(pos_def) then
            call strsv('L','T','N', nelim, lcol, blkm, xlocal, 1)
         else
            call strsv('L','T','U', nelim, lcol, blkm, xlocal, 1)
         end if
      end if
...
subroutine slv_bwd_mult(pos_def, job, rlist, invp, nrhs, x, ldx, blkm, &
   ...
   if (blkm.gt.10 .and. nelim.gt.4) then
      ...
      if (blkm-nelim.gt.0) then
         call sgemm('Trans', 'Non-trans', nelim, nrhs, blkm-nelim, -one, &
            lcol(nelim+1), blkm, xlocal(nelim+1,1), blkm, one, xlocal, &
            blkm)
      end if

      if(pos_def) then
         call strsm('Left','Lower','Trans','Non-Unit', nelim, nrhs, one, lcol, &
            blkm, xlocal, blkm)
      else
         call strsm('Left','Lower','Trans','Unit', nelim, nrhs, one, lcol, &
            blkm, xlocal, blkm)
      end if
      do r = 1, nrhs
         do i = 1, nelim
...
!
subroutine ma97_enquire_indef_single(akeep, fkeep, control, info, &
      piv_order, d)
   type(ma97_akeep), intent(in) :: akeep
   type(ma97_fkeep), target, intent(in) :: fkeep
   type(ma97_control), intent(in) :: control
...
subroutine finalise_both_single(akeep, fkeep)
   ...
   call free_akeep_single(akeep)
   call free_fkeep_single(fkeep)
...
!$ this_thread = omp_get_thread_num() + 1
   ...
   if (p .le. minsz .or. maxdepth.eq.1) then
      if(pos_def) then
         call spotrf('L', p, a, lda, lapack_info)
         if(lapack_info.ne.0) then
            stats(this_thread)%flag = MA97_ERROR_NOT_POS_DEF
            return
         end if
         call strsm('Right', 'Lower', 'Trans', 'Non-unit', n-p, p, &
            one, a, lda, a(p+1), lda)
         q = p
      else
         nullify(buf(this_thread)%chkptr)
      ...
      if (q1.ge.1) then
         if(pos_def) then
            call myssyrk(mu, nu, q1, -one, a(ip), lda, &
...
subroutine factor_solve_block(n, p, nb, s, a, lda, d, buf, perm,  &
   ...
   u = min(max(control%u,zero),0.5_wp)
   ...
   pivot: do
      ...
      sweep: do mdummy = 1, p-q ! Look for a pivotal column or pair of columns
         ...
         k = kkr1+m-rm-1
         if (q>rm) then
            !call sgemv('NoTrans',n-m+1,q-rm,-one,a(k),lda, &
            !   buf(lda*(rm+0_long)+m),lda,one,a(kkm),1)
            call mysgemv(n-m+1,q-rm,a(k),lda, &
      ...
      pivsizes: if (pivsiz.eq.1) then
         ...
         do j = q+2, min(nb,m)
            kkj = kkj + lda + 1
            call saxpy(n-j+1,-buf(kb1+j-q-1),a(kkq1+j-q-1),1,a(kkj),1)
         end do
         ! Update q and kkq1
         kkq1 = kkq1 + lda + 1
         q = q+1
         ...
         j = max(q+3,1)
         if (m>=j) then
            call sgemm('n','t',n-j+1,m-j+1,2,-one,a(kkq1+j-q-1), &
...
subroutine factor_solve_block_small(n, p, nb, s, a, lda, d, buf, perm,  &
   ...
   pivot: do
      ...
      pivsizes: if (pivsiz.eq.1) then
         ...
         j = max(q+3,1)
         if (p>=j) then
            call sgemm('n','t',n-j+1,p-j+1,2,-one,a(kkq1+j-q-1), &
               lda-((q+1)/nb)*nb,buf(kb1+j-q-1),ldbuf,one,a(kkj),lda)
         end if
         ! Update q and kkq1
         kkq1 = k + lda + 1
...
subroutine ma64_swap(n,nb,q,q1,rm,a,lda,aleft,aln,buf,ldbuf,perm,j1,j2)
   ...
   call sswap(q-q1+1-rm,buf(j1+rm*ldbuf),ldbuf,buf(j2+rm*ldbuf),ldbuf)
   !call sswap(q-q1+1,buf(j1),n,buf(j2),n)
   ...
   l = j1-1
   if (l>0) call sswap(l,a(k1),lda,a(k2),lda)
   if (aln>0) call sswap(aln,aleft(j1),lda,aleft(j2),lda)
   ...
   jb = j2-j1-1
   l = min(jb,nb-j1,j2-1)
   if (l>0.and.j2.ge.1) call sswap(l,a(k1),1,a(k2),lda)
   ...
   if (n>j2) call sswap(n-j2,a(k1),1,a(k2),1)
...
subroutine ma64_update(n,nb,kkq,a,lda,buf,ldbuf,jl,jr,pl,pr)
   ...
   do i = 1, j2-j+1, bs
      ...
      p = min(bs, j2-j+1-i+1)
      call sgemm ('n', 't', n-j+1-i+1, p, pr-pl+1, -one, &
...
!*******************************
! Equivalent to call:
! call sgemv('NoTrans', m, n, -one, a, lda, b, ldb, one, c, 1)
! However for small updates exploit explicit loops as this is faster
! (calls sgemv itself for large updates)
subroutine mysgemv(m, n, a, lda, b, ldb, c)
   integer, intent(in) :: m
   integer, intent(in) :: n
   ...
   select case(n)
   case(1)
      !call sgemv('NoTrans', m, 1, -one, a, lda, b, ldb, one, c, 1)
      b1 = b(1)
      c(1:m) = c(1:m) - b1 * a(1:m)
   case(2)
      !call sgemv('NoTrans', m, 2, -one, a, lda, b, ldb, one, c, 1)
      b1 = b(1)
      b2 = b(1+ldb)
