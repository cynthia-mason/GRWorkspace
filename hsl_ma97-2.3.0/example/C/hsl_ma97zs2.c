...
int main(void) {
   ...
   for(j=0; j<nrhs; j++) {
      if(flag_out[j]) {
         ...
         for(i=0; i<n; i++)
            printf(" (%5.3lf,%5.3lf)", creal(x[i+j*n]), cimag(x[i+j*n]));
      } else {
         printf("Right-hand side %d inconsistent. Ax=0, x^Tb/=0 given by:\n",
            j);
         for(i=0; i<n; i++)
            printf(" (%5.3lf,%5.3lf)", creal(x[nrhs*n+i+j*n]), cimag(x[nrhs*n+i+j*n]));
      }
      printf("\n");
   }
   ...
   for(j=0; j<nrhs; j++) {
      printf("S^{-1}PLX_%d = ", j);
      for(i=0; i<n; i++)
         printf(" (%5.3lf,%5.3lf)", creal(y[i+j*n]), cimag(y[i+j*n]));
      printf("\n");
   }
   ...
   for(i=0; i<nxi; i++) printf("%d (%5.3lf,%5.3lf)\n", xindex[i],
         creal(x[xindex[i]]), cimag(x[xindex[i]]));
