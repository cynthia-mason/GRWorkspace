function hsl_ma97_install(varargin)
%
%   hsl_ma97_install installs hsl_ma97.
%
%   VARARGIN
%
...
if (v(1)==7 && v(2) < 12)
   warning('Old matlab version - we only test against 7.11 (2011b) or newer');
end
...
% Includes 8-byte default integer and OpenMP
omplibs = '';
if(v(1)==7 && v(2) <= 11 && islinux)
   % LINUX g95 for 2010b and earlier
   fflags = ' FFLAGS="-i8 \$FFLAGS"'; % option for 64-bit default integer
   g95_fix = true;
else
   % LINUX gfortran for 2011a and above
   % MAC gfortran always
   if (v(1)==7 && v(2) < 12)
      % gfortran insufficient to support OpenMP 3.0
      fflags = ' FFLAGS="-fdefault-integer-8 \$FFLAGS"'; % option for 64-bit d.i.
   else
      % gfortran sufficient to support OpenMP 3.0
      % option for 64-bit d.i.
      fflags = ' FFLAGS="-fdefault-integer-8 -fopenmp \$FFLAGS"';
      omplibs = ' -lgomp';
   end
   g95_fix = false;
end
...
% Note: MEXFLAGS can override fflags setting!
if(size(strfind(strtrim(LIBS),'matlab')==0))
   LIBS = ['-lmwlapack -lmwblas' omplibs];
   nometis = 1; % true
   if(is64)
      MEXFLAGS = strcat(MEXFLAGS, fflags);
   end
else
   nometis = 0; % false
end
...
if(g95_fix)
   % work around another g95 bug
   do_unix_cmd('cp hsl_ma97d.g95.f90 hsl_ma97d.f90');
   do_unix_cmd('cp hsl_ma97z.g95.f90 hsl_ma97z.f90');
   mexcompile(MEXFLAGS, '-c', 'hsl_ma97d.f90', '')
   mexcompile(MEXFLAGS, '-c', 'hsl_ma97z.f90', '')
   do_unix_cmd('rm hsl_ma97d.f90 hsl_ma97z.f90');
else
   mexcompile(MEXFLAGS, '-c', [srcdir,'/hsl_ma97d.f90'], '')
   mexcompile(MEXFLAGS, '-c', [srcdir,'/hsl_ma97z.f90'], '')
end
mexcompile(MEXFLAGS, '-c', 'hsl_matlab.F90', '')
if(nometis)
   mexcompile(MEXFLAGS, '-c', [srcdir,'/fakemetis.f'], '')
   mexcompile(MEXFLAGS, '-output hsl_ma97_expert', 'hsl_ma97_expert.f90 hsl_matlab.o hsl_ma97d.o common90.o common.o ddeps90.o hsl_ma97z.o zdeps.o zdeps90.o fakemetis.o', LIBS)
else
   mexcompile(MEXFLAGS, '-output hsl_ma97_expert', 'hsl_ma97_expert.f90 hsl_matlab.o hsl_ma97d.o common90.o common.o ddeps90.o hsl_ma97z.o zdeps.o zdeps90.o', LIBS)
end
...
% Builds and executes string 'mex $(MEXFLAGS) $(output) $(files) $(LIBS)'
s = ['mex ', MEXFLAGS, ' ', output, ' ', files, ' ', LIBS];
fprintf('%s\n', s);
eval(s);
